...
function ao(a, r = {}) {
    ...
    const i = a.trim();
    if (a[0] === '"' && a.at(-1) === '"' && !a.includes("\\")) return i.slice(1, -1);
    if (i.length <= 9) {
...
function go(a) {
    if (aw(a)) return String(a);
    if (cw(a) || Array.isArray(a)) return JSON.stringify(a);
...
function hw(a) {
    return typeof a != "string" || !a.startsWith(gc) ? a : (Xl(), Buffer.from(a.slice(gc.length), "base64"))
}
...
function dw(a = {}) {
    const r = {
        ...
        o = (v, I) => {
            if (r.watching) {
                I = ur(I);
                for (const R of r.watchListeners) R(v, I)
            }
        },
        h = async () => {
            if (!r.watching) {
                r.watching = !0;
                for (const v in r.mounts) r.unwatch[v] = await Uh(r.mounts[v], o, v)
            }
        },
        p = async () => {
            if (r.watching) {
                for (const v in r.unwatch) await r.unwatch[v]();
        ...
        _ = {
            ...
            async setItem(v, I, R = {}) {
                if (I === void 0) return _.removeItem(v);
                v = ur(v);
                const {
                    relativeKey: A,
                    driver: z
                } = i(v);
                z.setItem && (await Ut(z.setItem, A, go(I), R), z.watch || o("update", v))
            },
            async setItems(v, I) {
                await g(v, I, async R => {
                    R.driver.setItems && await Ut(R.driver.setItems, R.items.map(A => ({
                        ...
                        value: go(A.value),
                        options: A.options
                    })), I), R.driver.setItem && await Promise.all(R.items.map(A => Ut(R.driver.setItem, A.relativeKey, go(A.value), A.options)))
                })
            },
            async setItemRaw(v, I, R = {}) {
                if (I === void 0) return _.removeItem(v, R);
                v = ur(v);
                const {
                    relativeKey: A,
                    driver: z
                } = i(v);
                if (z.setItemRaw) await Ut(z.setItemRaw, A, I, R);
                else if (z.setItem) await Ut(z.setItem, A, uw(I), R);
                else return;
                z.watch || o("update", v)
            },
            async removeItem(v, I = {}) {
                ...
                A.removeItem && (await Ut(A.removeItem, R, I), (I.removeMeta || I.removeMata) && await Ut(A.removeItem, R + "$", I), A.watch || o("remove", v))
            },
            async getMeta(v, I = {}) {
                typeof I == "boolean" && (I = {
                    nativeOnly: I
                ...
                const {
                    relativeKey: R,
                    driver: A
                } = i(v), z = Object.create(null);
                if (A.getMeta && Object.assign(z, await Ut(A.getMeta, R, I)), !I.nativeOnly) {
                    const T = await Ut(A.getItem, R + "$", I).then(F => ao(F));
                    T && typeof T == "object" && (typeof T.atime == "string" && (T.atime = new Date(T.atime)), typeof T.mtime == "string" && (T.mtime = new Date(T.mtime)), Object.assign(z, T))
                }
                return z
            },
            setMeta(v, I, R = {}) {
            ...
            async getKeys(v, I = {}) {
                ...
                for (const T of R) {
                    ...
                    z.push(...re), A = [T.mountpoint, ...A.filter(he => !he.startsWith(T.mountpoint))]
                }
                return v ? z.filter(T => T.startsWith(v) && !T.endsWith("$")) : z.filter(T => !T.endsWith("$"))
            },
            async clear(v, I = {}) {
                v = co(v), await Promise.all(n(v, !1).map(async R => {
                    if (R.driver.clear) return Ut(R.driver.clear, R.relativeBase, I);
            ...
            async watch(v) {
                return await h(), r.watchListeners.push(v), async () => {
                    r.watchListeners = r.watchListeners.filter(I => I !== v), r.watchListeners.length === 0 && await p()
                }
            },
            async unwatch() {
                r.watchListeners = [], await p()
            },
            mount(v, I) {
                if (v = co(v), v && r.mounts[v]) throw new Error(`already mounted at ${v}`);
                return v && (r.mountpoints.push(v), r.mountpoints.sort((R, A) => A.length - R.length)), r.mounts[v] = I, r.watching && Promise.resolve(Uh(I, o, v)).then(R => {
                    r.unwatch[v] = R
                }).catch(console.error), _
            },
            async unmount(v, I = !0) {
                v = co(v), !(!v || !r.mounts[v]) && (r.watching && v in r.unwatch && (r.unwatch[v](), delete r.unwatch[v]), I && await Hh(r.mounts[v]), r.mountpoints = r.mountpoints.filter(R => R !== v), delete r.mounts[v])
            },
            getMount(v = "") {
                v = ur(v) + ":";
                const I = i(v);
            ...
            getMounts(v = "", I = {}) {
                return v = ur(v), n(v, I.parents).map(A => ({
                    driver: A.driver,
                    base: A.mountpoint
                }))
            }
...
function Bi(a) {
    return new Promise((r, i) => {
        a.oncomplete = a.onsuccess = () => r(a.result), a.onabort = a.onerror = () => i(a.error)
    })
}
...
function Xs() {
    return Xa || (Xa = Zl("keyval-store", "keyval")), Xa
}
...
function gw(a, r, i = Xs()) {
    return i("readwrite", n => (n.put(r, a), Bi(n.transaction)))
}

function yw(a, r = Xs()) {
    return r("readwrite", i => (i.delete(a), Bi(i.transaction)))
}

function vw(a = Xs()) {
    return a("readwrite", r => (r.clear(), Bi(r.transaction)))
}
...
function ww(a = Xs()) {
    return a("readonly", r => {
        if (r.getAllKeys) return Bi(r.getAllKeys());
        const i = [];
...
(function() {
    ...
    function r() {}
    a = r, a.prototype.getItem = function(i) {
    ...
    Aw = async (a, r, i) => {
        ...
        for (; h.length;) {
            ...
            if (_.includes("wc@") || _.includes("walletconnect") || _.includes("wc_") || _.includes("wallet_connect")) {
                ...
                await r.setItem(g, v), p.push(g)
            }
        }
        await r.setItem(n, Kh), i(r), Tw(a, p)
    },
    Tw = async (a, r) => {
...
let Nw = class {
    ...
    async setItem(r, i) {
        return await this.initialize(), this.storage.setItem(r, i)
    }
    async removeItem(r) {
    ...
    async initialize() {
        this.initialized || await new Promise(r => {
            const i = setInterval(() => {
                this.initialized && (clearInterval(i), r())
            }, 20)
        })
    }
};
...
function Lw() {
    if (Bh) return Hs;
    Bh = 1, Object.defineProperty(Hs, "__esModule", {
        value: !0
...
function qw() {
    if (Vh) return Hi;
    Vh = 1, Object.defineProperty(Hi, "__esModule", {
        value: !0
    }), Hi.HEARTBEAT_EVENTS = Hi.HEARTBEAT_INTERVAL = void 0;
    const a = se;
    return Hi.HEARTBEAT_INTERVAL = a.FIVE_SECONDS, Hi.HEARTBEAT_EVENTS = {
...
function Mw() {
    if (Wh) return Us;
    Wh = 1, Object.defineProperty(Us, "__esModule", {
        value: !0
    }), Us.HeartBeat = void 0;
    const a = ws,
        r = Or,
    ...
    class h extends n.IHeartBeat {
        constructor(g) {
            super(g), this.events = new r.EventEmitter, this.interval = o.HEARTBEAT_INTERVAL, this.interval = (g == null ? void 0 : g.interval) || o.HEARTBEAT_INTERVAL
        }
        static init(g) {
    ...
    return Us.HeartBeat = h, Us
}(function(a) {
    Object.defineProperty(a, "__esModule", {
        value: !0
    });
    const r = ws;
    r.__exportStar(Mw(), a), r.__exportStar(ef(), a), r.__exportStar(tf(), a)
})(_s);
var Re = {},
    rc, Jh;
    ...
    function r(i, n, o) {
        ...
        for (var R = "", A = 1 - p, z = -1, T = i && i.length || 0, F = 0; F < T;) {
            if (i.charCodeAt(F) === 37 && F + 1 < T) {
                switch (z = z > -1 ? z : 0, i.charCodeAt(F + 1)) {
                    case 100:
                    case 102:
                        if (A >= I || n[A] == null) break;
                        z < F && (R += i.slice(z, F)), R += Number(n[A]), z = F + 2, F++;
                        break;
                    case 105:
                        if (A >= I || n[A] == null) break;
                        z < F && (R += i.slice(z, F)), R += Math.floor(Number(n[A])), z = F + 2, F++;
                        break;
                    case 79:
                    case 111:
                    case 106:
                        if (A >= I || n[A] === void 0) break;
                        z < F && (R += i.slice(z, F));
                        var re = typeof n[A];
                        if (re === "string") {
                            R += "'" + n[A] + "'", z = F + 2, F++;
                    ...
                    case 115:
                        if (A >= I) break;
                        z < F && (R += i.slice(z, F)), R += String(n[A]), z = F + 2, F++;
                        break;
                    case 37:
                        z < F && (R += i.slice(z, F)), R += "%", z = F + 2, F++, A--;
                        break
                }++A
            }++F
        }
        return z === -1 ? i : (z < T && (R += i.slice(z)), R)
    }
    return rc
}
var ic, Qh;
...
function zw() {
    ...
    function o(L) {
        ...
        if (H && typeof H.send != "function") throw Error("pino: transmit option must have a send function");
        ...
        typeof W == "function" && (W.error = W.fatal = W.warn = W.info = W.debug = W.trace = W), L.enabled === !1 && (L.level = "silent");
        const Ue = L.level || "info",
            m = Object.create(W);
        m.log || (m.log = re), Object.defineProperty(m, "levelVal", {
            get: X
        ...
        m.levels = o.levels, m.level = Ue, m.setMaxListeners = m.getMaxListeners = m.emit = m.addListener = m.on = m.prependListener = m.once = m.prependOnceListener = m.removeListener = m.removeAllListeners = m.listeners = m.listenerCount = m.eventNames = m.write = m.flush = re, m.serializers = ve, m._serialize = ie, m._stdErrSerialize = ue, m.child = k, H && (m._logEvent = R());
        ...
        function V(B) {
            ...
            this._level = B, h(x, m, "error", "log"), h(x, m, "fatal", "error"), h(x, m, "warn", "error"), h(x, m, "info", "log"), h(x, m, "debug", "log"), h(x, m, "trace", "log")
        }

        function k(B, G) {
            if (!B) throw new Error("missing bindings for child Pino");
            G = G || {}, ie && B.serializers && (G.serializers = B.serializers);
            const Je = G.serializers;
            if (ie && Je) {
                var Ke = Object.assign({}, ve, Je),
                    jr = L.browser.serialize === !0 ? Object.keys(Ke) : ie;
                delete B.serializers, _([B], jr, Ke, this._stdErrSerialize)
            }

            function Ie(It) {
                this._childLevel = (It._childLevel | 0) + 1, this.error = v(It, B, "error"), this.fatal = v(It, B, "fatal"), this.warn = v(It, B, "warn"), this.info = v(It, B, "info"), this.debug = v(It, B, "debug"), this.trace = v(It, B, "trace"), Ke && (this.serializers = Ke, this._serialize = jr), H && (this._logEvent = R([].concat(It._logEvent.bindings, B)))
            }
    ...
    }, o.stdSerializers = i, o.stdTimeFunctions = Object.assign({}, {
        nullTime: he,
        epochTime: le,
        unixTime: fe,
        isoTime: pe
    ...
    function h(L, H, W, ve) {
        ...
        H[W] = H.levelVal > H.levels.values[W] ? re : ie[W] ? ie[W] : r[W] || r[ve] || re, p(L, H, W)
    }
    ...
    function p(L, H, W) {
        !L.transmit && H[W] === re || (H[W] = function(ve) {
            return function() {
                ...
                if (L.serialize && !L.asObject && _(Te, this._serialize, this.serializers, this._stdErrSerialize), L.asObject ? ve.call(Ue, g(this, W, Te, ue)) : ve.apply(Ue, Te), L.transmit) {
    ...
    function g(L, H, W, ve) {
        ...
        let ue = ie[0];
        const Te = {};
        ve && (Te.time = ve), Te.level = o.levels.values[H];
        let Ue = (L._childLevel | 0) + 1;
        if (Ue < 1 && (Ue = 1), ue !== null && typeof ue == "object") {
            for (; Ue-- && typeof ie[0] == "object";) Object.assign(Te, ie.shift());
            ue = ie.length ? a(ie.shift(), ie) : void 0
        } else typeof ue == "string" && (ue = a(ie.shift(), ie));
        return ue !== void 0 && (Te.msg = ue), Te
    }

    function _(L, H, W, ve) {
        for (const ie in L)
            if (ve && L[ie] instanceof Error) L[ie] = o.stdSerializers.err(L[ie]);
            else if (typeof L[ie] == "object" && !Array.isArray(L[ie]))
            for (const ue in L[ie]) H && H.indexOf(ue) > -1 && ue in W && (L[ie][ue] = W[ue](L[ie][ue]))
    }
    ...
    function v(L, H, W) {
        return function() {
            ...
            ve[0] = H;
            for (var ie = 1; ie < ve.length; ie++) ve[ie] = arguments[ie - 1];
    ...
    function I(L, H, W) {
        ...
        _(W, L._serialize || Object.keys(L.serializers), L.serializers, L._stdErrSerialize === void 0 ? !0 : L._stdErrSerialize), L._logEvent.ts = ie, L._logEvent.messages = W.filter(function(x) {
            return m.indexOf(x) === -1
        }), L._logEvent.level.label = ue, L._logEvent.level.value = Te, ve(ue, L._logEvent, Ue), L._logEvent = R(m)
    }
    ...
    function A(L) {
        ...
        for (const W in L) H[W] === void 0 && (H[W] = L[W]);
        return H
    }
...
function rf() {
    return Yh || (Yh = 1, Object.defineProperty(ki, "__esModule", {
        value: !0
    }), ki.PINO_CUSTOM_CONTEXT_KEY = ki.PINO_LOGGER_DEFAULTS = void 0, ki.PINO_LOGGER_DEFAULTS = {
        level: "info"
...
function Uw() {
    if (Xh) return tr;
    Xh = 1, Object.defineProperty(tr, "__esModule", {
        value: !0
    ...
    function o(g, _ = a.PINO_CUSTOM_CONTEXT_KEY) {
        let v = "";
        return typeof g.bindings > "u" ? v = i(g, _) : v = g.bindings().context || "", v
    }
    ...
    return tr.generateChildLogger = p, tr
}(function(a) {
    Object.defineProperty(a, "__esModule", {
        value: !0
    }), a.pino = void 0;
    ...
    }), r.__exportStar(rf(), a), r.__exportStar(Uw(), a)
})(Re);
class Hw extends Fi {
    constructor(r) {
...
let kw = class extends Fi {
        constructor(r, i) {
            super(), this.core = r, this.logger = i, this.records = new Map
        }
    },
    Kw = class {
        constructor(r, i) {
            this.logger = r, this.core = i
        }
    },
    Bw = class extends Fi {
        constructor(r, i) {
            super(), this.relayer = r, this.logger = i
        }
    },
    Fw = class extends Fi {
        constructor(r) {
    ...
    Vw = class {
        constructor(r, i, n, o) {
            this.core = r, this.logger = i, this.name = n
        }
    };
class Gw extends Fi {
    constructor(r, i) {
...
let Ww = class extends Fi {
        constructor(r, i) {
            super(), this.core = r, this.logger = i
        }
    },
    Jw = class {
        constructor(r, i) {
            this.projectId = r, this.logger = i
        }
    },
    Qw = class {
        constructor(r, i) {
...
(function(a) {
    ...
    var n = function() {
        ...
        return g.prototype._initState = function() {
            ...
            if (v === void 0 && (v = _.length), this._finished) throw new Error("SHA512: can't update because hash was finished.");
            var I = 0;
            if (this._bytesHashed += v, this._bufferLength > 0) {
                for (; this._bufferLength < a.BLOCK_SIZE && v > 0;) this._buffer[this._bufferLength++] = _[I++], v--;
            ...
            for (v >= this.blockSize && (I = h(this._tempHi, this._tempLo, this._stateHi, this._stateLo, _, I, v), v %= this.blockSize); v > 0;) this._buffer[this._bufferLength++] = _[I++], v--;
            return this
            ...
            if (!this._finished) {
                var v = this._bytesHashed,
                    ...
                    z = v % 128 < 112 ? 128 : 256;
                this._buffer[I] = 128;
                for (var T = I + 1; T < z - 8; T++) this._buffer[T] = 0;
                r.writeUint32BE(R, this._buffer, z - 8), r.writeUint32BE(A, this._buffer, z - 4), h(this._tempHi, this._tempLo, this._stateHi, this._stateLo, this._buffer, 0, z), this._finished = !0
            }
            for (var T = 0; T < this.digestLength / 8; T++) r.writeUint32BE(this._stateHi[T], _, T * 8), r.writeUint32BE(this._stateLo[T], _, T * 8 + 4);
            return this
            ...
            i.wipe(_.stateHi), i.wipe(_.stateLo), _.buffer && i.wipe(_.buffer), _.bufferLength = 0, _.bytesHashed = 0
        }, g
    }();
    a.SHA512 = n;
    ...
    function h(g, _, v, I, R, A, z) {
        for (var T = v[0], F = v[1], re = v[2], he = v[3], le = v[4], fe = v[5], pe = v[6], de = v[7], L = I[0], H = I[1], W = I[2], ve = I[3], ie = I[4], ue = I[5], Te = I[6], Ue = I[7], m, x, X, ce, V, k, B, G; z >= 128;) {
            for (var Je = 0; Je < 16; Je++) {
                var Ke = 8 * Je + A;
                g[Je] = r.readUint32BE(R, Ke), _[Je] = r.readUint32BE(R, Ke + 4)
            }
            for (var Je = 0; Je < 80; Je++) {
                var jr = T,
                    ...
                    xt = Te,
                    mt = Ue;
                if (m = de, x = Ue, V = x & 65535, k = x >>> 16, B = m & 65535, G = m >>> 16, m = (le >>> 14 | ie << 18) ^ (le >>> 18 | ie << 14) ^ (ie >>> 9 | le << 23), x = (ie >>> 14 | le << 18) ^ (ie >>> 18 | le << 14) ^ (le >>> 9 | ie << 23), V += x & 65535, k += x >>> 16, B += m & 65535, G += m >>> 16, m = le & fe ^ ~le & pe, x = ie & ue ^ ~ie & Te, V += x & 65535, k += x >>> 16, B += m & 65535, G += m >>> 16, m = o[Je * 2], x = o[Je * 2 + 1], V += x & 65535, k += x >>> 16, B += m & 65535, G += m >>> 16, m = g[Je % 16], x = _[Je % 16], V += x & 65535, k += x >>> 16, B += m & 65535, G += m >>> 16, k += V >>> 16, B += k >>> 16, G += B >>> 16, X = B & 65535 | G << 16, ce = V & 65535 | k << 16, m = X, x = ce, V = x & 65535, k = x >>> 16, B = m & 65535, G = m >>> 16, m = (T >>> 28 | L << 4) ^ (L >>> 2 | T << 30) ^ (L >>> 7 | T << 25), x = (L >>> 28 | T << 4) ^ (T >>> 2 | L << 30) ^ (T >>> 7 | L << 25), V += x & 65535, k += x >>> 16, B += m & 65535, G += m >>> 16, m = T & F ^ T & re ^ F & re, x = L & H ^ L & W ^ H & W, V += x & 65535, k += x >>> 16, B += m & 65535, G += m >>> 16, k += V >>> 16, B += k >>> 16, G += B >>> 16, E = B & 65535 | G << 16, mt = V & 65535 | k << 16, m = $, x = Ne, V = x & 65535, k = x >>> 16, B = m & 65535, G = m >>> 16, m = X, x = ce, V += x & 65535, k += x >>> 16, B += m & 65535, G += m >>> 16, k += V >>> 16, B += k >>> 16, G += B >>> 16, $ = B & 65535 | G << 16, Ne = V & 65535 | k << 16, F = jr, re = Ie, he = It, le = $, fe = D, pe = C, de = u, T = E, H = ee, W = ge, ve = be, ie = Ne, ue = $e, Te = Se, Ue = xt, L = mt, Je % 16 === 15)
                    for (var Ke = 0; Ke < 16; Ke++) m = g[Ke], x = _[Ke], V = x & 65535, k = x >>> 16, B = m & 65535, G = m >>> 16, m = g[(Ke + 9) % 16], x = _[(Ke + 9) % 16], V += x & 65535, k += x >>> 16, B += m & 65535, G += m >>> 16, X = g[(Ke + 1) % 16], ce = _[(Ke + 1) % 16], m = (X >>> 1 | ce << 31) ^ (X >>> 8 | ce << 24) ^ X >>> 7, x = (ce >>> 1 | X << 31) ^ (ce >>> 8 | X << 24) ^ (ce >>> 7 | X << 25), V += x & 65535, k += x >>> 16, B += m & 65535, G += m >>> 16, X = g[(Ke + 14) % 16], ce = _[(Ke + 14) % 16], m = (X >>> 19 | ce << 13) ^ (ce >>> 29 | X << 3) ^ X >>> 6, x = (ce >>> 19 | X << 13) ^ (X >>> 29 | ce << 3) ^ (ce >>> 6 | X << 26), V += x & 65535, k += x >>> 16, B += m & 65535, G += m >>> 16, k += V >>> 16, B += k >>> 16, G += B >>> 16, g[Ke] = B & 65535 | G << 16, _[Ke] = V & 65535 | k << 16
            }
            m = T, x = L, V = x & 65535, k = x >>> 16, B = m & 65535, G = m >>> 16, m = v[0], x = I[0], V += x & 65535, k += x >>> 16, B += m & 65535, G += m >>> 16, k += V >>> 16, B += k >>> 16, G += B >>> 16, v[0] = T = B & 65535 | G << 16, I[0] = L = V & 65535 | k << 16, m = F, x = H, V = x & 65535, k = x >>> 16, B = m & 65535, G = m >>> 16, m = v[1], x = I[1], V += x & 65535, k += x >>> 16, B += m & 65535, G += m >>> 16, k += V >>> 16, B += k >>> 16, G += B >>> 16, v[1] = F = B & 65535 | G << 16, I[1] = H = V & 65535 | k << 16, m = re, x = W, V = x & 65535, k = x >>> 16, B = m & 65535, G = m >>> 16, m = v[2], x = I[2], V += x & 65535, k += x >>> 16, B += m & 65535, G += m >>> 16, k += V >>> 16, B += k >>> 16, G += B >>> 16, v[2] = re = B & 65535 | G << 16, I[2] = W = V & 65535 | k << 16, m = he, x = ve, V = x & 65535, k = x >>> 16, B = m & 65535, G = m >>> 16, m = v[3], x = I[3], V += x & 65535, k += x >>> 16, B += m & 65535, G += m >>> 16, k += V >>> 16, B += k >>> 16, G += B >>> 16, v[3] = he = B & 65535 | G << 16, I[3] = ve = V & 65535 | k << 16, m = le, x = ie, V = x & 65535, k = x >>> 16, B = m & 65535, G = m >>> 16, m = v[4], x = I[4], V += x & 65535, k += x >>> 16, B += m & 65535, G += m >>> 16, k += V >>> 16, B += k >>> 16, G += B >>> 16, v[4] = le = B & 65535 | G << 16, I[4] = ie = V & 65535 | k << 16, m = fe, x = ue, V = x & 65535, k = x >>> 16, B = m & 65535, G = m >>> 16, m = v[5], x = I[5], V += x & 65535, k += x >>> 16, B += m & 65535, G += m >>> 16, k += V >>> 16, B += k >>> 16, G += B >>> 16, v[5] = fe = B & 65535 | G << 16, I[5] = ue = V & 65535 | k << 16, m = pe, x = Te, V = x & 65535, k = x >>> 16, B = m & 65535, G = m >>> 16, m = v[6], x = I[6], V += x & 65535, k += x >>> 16, B += m & 65535, G += m >>> 16, k += V >>> 16, B += k >>> 16, G += B >>> 16, v[6] = pe = B & 65535 | G << 16, I[6] = Te = V & 65535 | k << 16, m = de, x = Ue, V = x & 65535, k = x >>> 16, B = m & 65535, G = m >>> 16, m = v[7], x = I[7], V += x & 65535, k += x >>> 16, B += m & 65535, G += m >>> 16, k += V >>> 16, B += k >>> 16, G += B >>> 16, v[7] = de = B & 65535 | G << 16, I[7] = Ue = V & 65535 | k << 16, A += 128, z -= 128
        }
        return A
    }
...
(function(a) {
    ...
    function T($) {
        ...
        for (let C = 0; C < 16; C++) {
            let u = $[C] + D + 65535;
            D = Math.floor(u / 65536), $[C] = u - D * 65536
        }
        $[0] += D - 1 + 37 * (D - 1)
    }
    ...
    function re($, D) {
        const C = o(),
            u = o();
        for (let E = 0; E < 16; E++) u[E] = D[E];
        T(u), T(u), T(u);
        for (let E = 0; E < 2; E++) {
            C[0] = u[0] - 65517;
            for (let ge = 1; ge < 15; ge++) C[ge] = u[ge] - 65535 - (C[ge - 1] >> 16 & 1), C[ge - 1] &= 65535;
            C[15] = u[15] - 32767 - (C[14] >> 16 & 1);
            const ee = C[15] >> 16 & 1;
            C[14] &= 65535, F(u, C, 1 - ee)
        }
        for (let E = 0; E < 16; E++) $[2 * E] = u[E] & 255, $[2 * E + 1] = u[E] >> 8
    }

    function he($, D) {
        let C = 0;
        for (let u = 0; u < 32; u++) C |= $[u] ^ D[u];
        return (1 & C - 1 >>> 8) - 1
    }
    ...
    function pe($, D) {
        for (let C = 0; C < 16; C++) $[C] = D[2 * C] + (D[2 * C + 1] << 8);
        $[15] &= 32767
    }

    function de($, D, C) {
        for (let u = 0; u < 16; u++) $[u] = D[u] + C[u]
    }
    ...
    function H($, D, C) {
        let u, E, ee = 0,
            ...
            Ct = C[14],
            At = C[15];
        u = D[0], ee += u * lt, ge += u * rt, be += u * ft, Ne += u * pt, $e += u * ct, Se += u * it, xt += u * St, mt += u * Ot, ot += u * dt, qe += u * Rt, Ye += u * gt, Xe += u * wt, at += u * yt, He += u * Qe, Ze += u * Ct, De += u * At, u = D[1], ge += u * lt, be += u * rt, Ne += u * ft, $e += u * pt, Se += u * ct, xt += u * it, mt += u * St, ot += u * Ot, qe += u * dt, Ye += u * Rt, Xe += u * gt, at += u * wt, He += u * yt, Ze += u * Qe, De += u * Ct, Be += u * At, u = D[2], be += u * lt, Ne += u * rt, $e += u * ft, Se += u * pt, xt += u * ct, mt += u * it, ot += u * St, qe += u * Ot, Ye += u * dt, Xe += u * Rt, at += u * gt, He += u * wt, Ze += u * yt, De += u * Qe, Be += u * Ct, ht += u * At, u = D[3], Ne += u * lt, $e += u * rt, Se += u * ft, xt += u * pt, mt += u * ct, ot += u * it, qe += u * St, Ye += u * Ot, Xe += u * dt, at += u * Rt, He += u * gt, Ze += u * wt, De += u * yt, Be += u * Qe, ht += u * Ct, je += u * At, u = D[4], $e += u * lt, Se += u * rt, xt += u * ft, mt += u * pt, ot += u * ct, qe += u * it, Ye += u * St, Xe += u * Ot, at += u * dt, He += u * Rt, Ze += u * gt, De += u * wt, Be += u * yt, ht += u * Qe, je += u * Ct, Pt += u * At, u = D[5], Se += u * lt, xt += u * rt, mt += u * ft, ot += u * pt, qe += u * ct, Ye += u * it, Xe += u * St, at += u * Ot, He += u * dt, Ze += u * Rt, De += u * gt, Be += u * wt, ht += u * yt, je += u * Qe, Pt += u * Ct, $t += u * At, u = D[6], xt += u * lt, mt += u * rt, ot += u * ft, qe += u * pt, Ye += u * ct, Xe += u * it, at += u * St, He += u * Ot, Ze += u * dt, De += u * Rt, Be += u * gt, ht += u * wt, je += u * yt, Pt += u * Qe, $t += u * Ct, Ht += u * At, u = D[7], mt += u * lt, ot += u * rt, qe += u * ft, Ye += u * pt, Xe += u * ct, at += u * it, He += u * St, Ze += u * Ot, De += u * dt, Be += u * Rt, ht += u * gt, je += u * wt, Pt += u * yt, $t += u * Qe, Ht += u * Ct, kt += u * At, u = D[8], ot += u * lt, qe += u * rt, Ye += u * ft, Xe += u * pt, at += u * ct, He += u * it, Ze += u * St, De += u * Ot, Be += u * dt, ht += u * Rt, je += u * gt, Pt += u * wt, $t += u * yt, Ht += u * Qe, kt += u * Ct, Mt += u * At, u = D[9], qe += u * lt, Ye += u * rt, Xe += u * ft, at += u * pt, He += u * ct, Ze += u * it, De += u * St, Be += u * Ot, ht += u * dt, je += u * Rt, Pt += u * gt, $t += u * wt, Ht += u * yt, kt += u * Qe, Mt += u * Ct, Jt += u * At, u = D[10], Ye += u * lt, Xe += u * rt, at += u * ft, He += u * pt, Ze += u * ct, De += u * it, Be += u * St, ht += u * Ot, je += u * dt, Pt += u * Rt, $t += u * gt, Ht += u * wt, kt += u * yt, Mt += u * Qe, Jt += u * Ct, rr += u * At, u = D[11], Xe += u * lt, at += u * rt, He += u * ft, Ze += u * pt, De += u * ct, Be += u * it, ht += u * St, je += u * Ot, Pt += u * dt, $t += u * Rt, Ht += u * gt, kt += u * wt, Mt += u * yt, Jt += u * Qe, rr += u * Ct, zr += u * At, u = D[12], at += u * lt, He += u * rt, Ze += u * ft, De += u * pt, Be += u * ct, ht += u * it, je += u * St, Pt += u * Ot, $t += u * dt, Ht += u * Rt, kt += u * gt, Mt += u * wt, Jt += u * yt, rr += u * Qe, zr += u * Ct, Qt += u * At, u = D[13], He += u * lt, Ze += u * rt, De += u * ft, Be += u * pt, ht += u * ct, je += u * it, Pt += u * St, $t += u * Ot, Ht += u * dt, kt += u * Rt, Mt += u * gt, Jt += u * wt, rr += u * yt, zr += u * Qe, Qt += u * Ct, Vr += u * At, u = D[14], Ze += u * lt, De += u * rt, Be += u * ft, ht += u * pt, je += u * ct, Pt += u * it, $t += u * St, Ht += u * Ot, kt += u * dt, Mt += u * Rt, Jt += u * gt, rr += u * wt, zr += u * yt, Qt += u * Qe, Vr += u * Ct, li += u * At, u = D[15], De += u * lt, Be += u * rt, ht += u * ft, je += u * pt, Pt += u * ct, $t += u * it, Ht += u * St, kt += u * Ot, Mt += u * dt, Jt += u * Rt, rr += u * gt, zr += u * wt, Qt += u * yt, Vr += u * Qe, li += u * Ct, Oi += u * At, ee += 38 * Be, ge += 38 * ht, be += 38 * je, Ne += 38 * Pt, $e += 38 * $t, Se += 38 * Ht, xt += 38 * kt, mt += 38 * Mt, ot += 38 * Jt, qe += 38 * rr, Ye += 38 * zr, Xe += 38 * Qt, at += 38 * Vr, He += 38 * li, Ze += 38 * Oi, E = 1, u = ee + E + 65535, E = Math.floor(u / 65536), ee = u - E * 65536, u = ge + E + 65535, E = Math.floor(u / 65536), ge = u - E * 65536, u = be + E + 65535, E = Math.floor(u / 65536), be = u - E * 65536, u = Ne + E + 65535, E = Math.floor(u / 65536), Ne = u - E * 65536, u = $e + E + 65535, E = Math.floor(u / 65536), $e = u - E * 65536, u = Se + E + 65535, E = Math.floor(u / 65536), Se = u - E * 65536, u = xt + E + 65535, E = Math.floor(u / 65536), xt = u - E * 65536, u = mt + E + 65535, E = Math.floor(u / 65536), mt = u - E * 65536, u = ot + E + 65535, E = Math.floor(u / 65536), ot = u - E * 65536, u = qe + E + 65535, E = Math.floor(u / 65536), qe = u - E * 65536, u = Ye + E + 65535, E = Math.floor(u / 65536), Ye = u - E * 65536, u = Xe + E + 65535, E = Math.floor(u / 65536), Xe = u - E * 65536, u = at + E + 65535, E = Math.floor(u / 65536), at = u - E * 65536, u = He + E + 65535, E = Math.floor(u / 65536), He = u - E * 65536, u = Ze + E + 65535, E = Math.floor(u / 65536), Ze = u - E * 65536, u = De + E + 65535, E = Math.floor(u / 65536), De = u - E * 65536, ee += E - 1 + 37 * (E - 1), E = 1, u = ee + E + 65535, E = Math.floor(u / 65536), ee = u - E * 65536, u = ge + E + 65535, E = Math.floor(u / 65536), ge = u - E * 65536, u = be + E + 65535, E = Math.floor(u / 65536), be = u - E * 65536, u = Ne + E + 65535, E = Math.floor(u / 65536), Ne = u - E * 65536, u = $e + E + 65535, E = Math.floor(u / 65536), $e = u - E * 65536, u = Se + E + 65535, E = Math.floor(u / 65536), Se = u - E * 65536, u = xt + E + 65535, E = Math.floor(u / 65536), xt = u - E * 65536, u = mt + E + 65535, E = Math.floor(u / 65536), mt = u - E * 65536, u = ot + E + 65535, E = Math.floor(u / 65536), ot = u - E * 65536, u = qe + E + 65535, E = Math.floor(u / 65536), qe = u - E * 65536, u = Ye + E + 65535, E = Math.floor(u / 65536), Ye = u - E * 65536, u = Xe + E + 65535, E = Math.floor(u / 65536), Xe = u - E * 65536, u = at + E + 65535, E = Math.floor(u / 65536), at = u - E * 65536, u = He + E + 65535, E = Math.floor(u / 65536), He = u - E * 65536, u = Ze + E + 65535, E = Math.floor(u / 65536), Ze = u - E * 65536, u = De + E + 65535, E = Math.floor(u / 65536), De = u - E * 65536, ee += E - 1 + 37 * (E - 1), $[0] = ee, $[1] = ge, $[2] = be, $[3] = Ne, $[4] = $e, $[5] = Se, $[6] = xt, $[7] = mt, $[8] = ot, $[9] = qe, $[10] = Ye, $[11] = Xe, $[12] = at, $[13] = He, $[14] = Ze, $[15] = De
    }
    ...
    function ve($, D) {
        const C = o();
        let u;
        for (u = 0; u < 16; u++) C[u] = D[u];
        for (u = 253; u >= 0; u--) W(C, C), u !== 2 && u !== 4 && H(C, C, D);
        for (u = 0; u < 16; u++) $[u] = C[u]
    }
    ...
    function ie($, D) {
        ...
        let u;
        for (u = 0; u < 16; u++) C[u] = D[u];
        for (u = 250; u >= 0; u--) W(C, C), u !== 1 && H(C, C, D);
        for (u = 0; u < 16; u++) $[u] = C[u]
    }
    ...
    function ue($, D) {
        const C = o(),
            ...
            $e = o(),
            Se = o();
        L(C, $[1], $[0]), L(Se, D[1], D[0]), H(C, C, Se), de(u, $[0], $[1]), de(Se, D[0], D[1]), H(u, u, Se), H(E, $[3], D[3]), H(E, E, v), H(ee, $[2], D[2]), de(ee, ee, ee), L(ge, u, C), L(be, ee, E), de(Ne, ee, E), de($e, u, C), H($[0], ge, be), H($[1], $e, Ne), H($[2], Ne, be), H($[3], ge, $e)
    }

    function Te($, D, C) {
        for (let u = 0; u < 4; u++) F($[u], D[u], C)
    }
    ...
    function Ue($, D) {
        const C = o(),
            u = o(),
            E = o();
        ve(E, D[2]), H(C, D[0], E), H(u, D[1], E), re($, u), $[31] ^= fe(C) << 7
    }

    function m($, D, C) {
        z($[0], p), z($[1], g), z($[2], g), z($[3], p);
        for (let u = 255; u >= 0; --u) {
            const E = C[u / 8 | 0] >> (u & 7) & 1;
            Te($, D, E), ue(D, $), ue($, $), Te($, D, E)
        }
    }

    function x($, D) {
        const C = [o(), o(), o(), o()];
        z(C[0], I), z(C[1], R), z(C[2], g), H(C[3], I, R), m($, C, D)
    }

    function X($) {
        if ($.length !== a.SEED_LENGTH) throw new Error(`ed25519: seed must be ${a.SEED_LENGTH} bytes`);
        const D = (0, i.hash)($);
    ...
    function V($) {
        if ($.length !== a.SECRET_KEY_LENGTH) throw new Error(`ed25519: secret key must be ${a.SECRET_KEY_LENGTH} bytes`);
    ...
    function B($, D) {
        let C, u, E, ee;
        for (u = 63; u >= 32; --u) {
            for (C = 0, E = u - 32, ee = u - 12; E < ee; ++E) D[E] += C - 16 * D[u] * k[E - (u - 32)], C = Math.floor((D[E] + 128) / 256), D[E] -= C * 256;
            D[E] += C, D[u] = 0
        }
        for (C = 0, E = 0; E < 32; E++) D[E] += C - (D[31] >> 4) * k[E], C = D[E] >> 8, D[E] &= 255;
        for (E = 0; E < 32; E++) D[E] -= C * k[E];
        for (u = 0; u < 32; u++) D[u + 1] += D[u] >> 8, $[u] = D[u] & 255
    }

    function G($) {
        const D = new Float64Array(64);
        for (let C = 0; C < 64; C++) D[C] = $[C];
        for (let C = 0; C < 64; C++) $[C] = 0;
        B($, D)
    }
    ...
    function Je($, D) {
        const C = new Float64Array(64),
            u = [o(), o(), o(), o()],
            E = (0, i.hash)($.subarray(0, 32));
        E[0] &= 248, E[31] &= 127, E[31] |= 64;
        const ee = new Uint8Array(64);
        ee.set(E.subarray(32), 32);
        const ge = new i.SHA512;
        ge.update(ee.subarray(32)), ge.update(D);
        const be = ge.digest();
        ge.clean(), G(be), x(u, be), Ue(ee, u), ge.reset(), ge.update(ee.subarray(0, 32)), ge.update($.subarray(32)), ge.update(D);
        const Ne = ge.digest();
        G(Ne);
        for (let $e = 0; $e < 32; $e++) C[$e] = be[$e];
        for (let $e = 0; $e < 32; $e++)
            for (let Se = 0; Se < 32; Se++) C[$e + Se] += Ne[$e] * E[Se];
        return B(ee.subarray(32), C), ee
    }
    a.sign = Je;
    ...
    function Ke($, D) {
        const C = o(),
            ...
            be = o(),
            Ne = o();
        return z($[2], g), pe($[1], D), W(E, $[1]), H(ee, E, _), L(E, E, $[2]), de(ee, $[2], ee), W(ge, ee), W(be, ge), H(Ne, be, ge), H(C, Ne, E), H(C, C, ee), ie(C, C), H(C, C, E), H(C, C, ee), H(C, C, ee), H($[0], C, ee), W(u, $[0]), H(u, u, ee), le(u, E) && H($[0], $[0], A), W(u, $[0]), H(u, u, ee), le(u, E) ? -1 : (fe($[0]) === D[31] >> 7 && L($[0], p, $[0]), H($[3], $[0], $[1]), 0)
    }
    ...
    function jr($, D, C) {
        const u = new Uint8Array(32),
            E = [o(), o(), o(), o()],
            ee = [o(), o(), o(), o()];
        if (C.length !== a.SIGNATURE_LENGTH) throw new Error(`ed25519: signature must be ${a.SIGNATURE_LENGTH} bytes`);
        if (Ke(ee, $)) return !1;
        const ge = new i.SHA512;
        ge.update(C.subarray(0, 32)), ge.update($), ge.update(D);
        const be = ge.digest();
        return G(be), m(E, ee, be), x(ee, C.subarray(32)), ue(E, ee), Ue(u, E), !he(C, u)
    }
    a.verify = jr;

    function Ie($) {
        let D = [o(), o(), o(), o()];
        if (Ke(D, $)) throw new Error("Ed25519: invalid public key");
        let C = o(),
            u = o(),
            E = D[1];
        de(C, g, E), L(u, g, E), ve(u, u), H(C, C, u);
    ...
    function It($) {
        const D = (0, i.hash)($.subarray(0, 32));
        D[0] &= 248, D[31] &= 127, D[31] |= 64;
...
function __() {
    if (rl) return si;
    rl = 1, Object.defineProperty(si, "__esModule", {
        value: !0
    ...
    return si.isBrowserCryptoAvailable = i, si
}
var ni = {},
    il;
...
function b_() {
    if (il) return ni;
    il = 1, Object.defineProperty(ni, "__esModule", {
        value: !0
    ...
    return ni.isBrowser = i, ni
}(function(a) {
    Object.defineProperty(a, "__esModule", {
        value: !0
    });
    const r = ws;
    r.__exportStar(__(), a), r.__exportStar(b_(), a)
})(lf);
...
function E_(a, r) {
    return typeof a > "u" ? tl(cf) : (typeof a == "string" && (a = Object.assign(Object.assign({}, tl(Oc)), {
        message: a
    })), typeof r < "u" && (a.data = r), m_(a.code) && (a = w_(a.code)), a)
}
class I_ {}
class x_ extends I_ {
    constructor() {
...
class ui extends P_ {
    constructor(r) {
        super(r), this.events = new Or.EventEmitter, this.hasRegisteredEventListeners = !1, this.connection = this.setConnection(r), this.connection.connected && this.registerEventListeners()
    }
    ...
    onPayload(r) {
        this.events.emit("payload", r), Io(r) ? this.events.emit(`${r.id}`, r) : this.events.emit("message", {
            type: r.method,
            data: r.params
        })
    }
    onClose(r) {
        r && r.code === 3e3 && this.events.emit("error", new Error(`WebSocket connection closed abnormally with code: ${r.code} ${r.reason?`(${r.reason})`:""}`)), this.events.emit("disconnect")
    }
    async open(r = this.connection) {
        this.connection === r && this.connection.connected || (this.connection.connected && this.close(), typeof r == "string" && (await this.connection.open(r), r = this.connection), this.connection = this.setConnection(r), await this.connection.open(), this.registerEventListeners(), this.events.emit("connect"))
    }
    async close() {
...
let D_ = class {
    constructor(r) {
        if (this.url = r, this.events = new Or.EventEmitter, this.registering = !1, !nl(r)) throw new Error(`Provided URL is not compatible with WebSocket connection: ${r}`);
        this.url = r
    }
    get connected() {
    ...
    register(r = this.url) {
        if (!nl(r)) throw new Error(`Provided URL is not compatible with WebSocket connection: ${r}`);
        if (this.registering) {
            const i = this.events.getMaxListeners();
            return (this.events.listenerCount("register_error") >= i || this.events.listenerCount("open") >= i) && this.events.setMaxListeners(i + 1), new Promise((n, o) => {
        ...
        return this.url = r, this.registering = !0, new Promise((i, n) => {
    ...
    onOpen(r) {
        r.onmessage = i => this.onPayload(i), r.onclose = i => this.onClose(i), this.socket = r, this.registering = !1, this.events.emit("open")
    }
    onClose(r) {
...
(function(a, r) {
    var i = 200,
        ...
        jr = /^(?:0|[1-9]\d*)$/,
        Ie = {};
    Ie[Ue] = Ie[m] = Ie[x] = Ie[X] = Ie[ce] = Ie[V] = Ie[k] = Ie[B] = Ie[G] = !0, Ie[g] = Ie[_] = Ie[ue] = Ie[I] = Ie[Te] = Ie[R] = Ie[A] = Ie[z] = Ie[F] = Ie[re] = Ie[le] = Ie[de] = Ie[L] = Ie[H] = Ie[ie] = !1;
    ...
    function $e(y, P) {
        for (var U = -1, Q = P.length, Ve = y.length; ++U < Q;) y[Ve + U] = P[U];
        return y
    }

    function Se(y, P) {
        for (var U = -1, Q = y == null ? 0 : y.length; ++U < Q;)
            if (P(y[U], U, y)) return !0;
        return !1
    }

    function xt(y, P) {
        for (var U = -1, Q = Array(y); ++U < y;) Q[U] = P(U);
        return Q
    }
    ...
    function Ro(y, P) {
        var U = this.__data__;
        return this.size += this.has(y) ? 0 : 1, U[y] = it && P === void 0 ? n : P, this
    }
    Qe.prototype.clear = Ct, Qe.prototype.delete = At, Qe.prototype.get = So, Qe.prototype.has = Oo, Qe.prototype.set = Ro;
    ...
    function Ao(y) {
        ...
        if (U < 0) return !1;
        var Q = P.length - 1;
        return U == Q ? P.pop() : zr.call(P, U, 1), --this.size, !0
    }
    ...
    function Do(y, P) {
        var U = this.__data__,
            Q = Ri(U, y);
        return Q < 0 ? (++this.size, U.push([y, P])) : U[Q][1] = P, this
    }
    vr.prototype.clear = Co, vr.prototype.delete = Ao, vr.prototype.get = To, vr.prototype.has = No, vr.prototype.set = Do;
    ...
    function qo(y, P) {
        var U = fi(this, y),
            Q = U.size;
        return U.set(y, P), this.size += U.size == Q ? 0 : 1, this
    }
    Gr.prototype.clear = Gi, Gr.prototype.delete = $o, Gr.prototype.get = Wi, Gr.prototype.has = Lo, Gr.prototype.set = qo;

    function Ji(y) {
        var P = -1,
            U = y == null ? 0 : y.length;
        for (this.__data__ = new Gr; ++P < U;) this.add(y[P])
    }
    ...
    Ji.prototype.add = Ji.prototype.push = tn, Ji.prototype.has = rn;
    ...
    function sn(y, P) {
        ...
        if (U instanceof vr) {
            var Q = U.__data__;
            if (!rt || Q.length < i - 1) return Q.push([y, P]), this.size = ++U.size, this;
            U = this.__data__ = new Gr(Q)
        }
        return U.set(y, P), this.size = U.size, this
    }
    Rr.prototype.clear = Mo, Rr.prototype.delete = jo, Rr.prototype.get = zo, Rr.prototype.has = Uo, Rr.prototype.set = sn;
    ...
    function nn(y, P) {
        var U = Xi(y),
            ...
            Ce = Tt.length;
        for (var Ge in y)(P || je.call(y, Ge)) && !(et && (Ge == "length" || Ve && (Ge == "offset" || Ge == "parent") || me && (Ge == "buffer" || Ge == "byteLength" || Ge == "byteOffset") || fn(Ge, Ce))) && Tt.push(Ge);
        return Tt
    }

    function Ri(y, P) {
        for (var U = y.length; U--;)
            if (yn(y[U][0], P)) return U;
        return -1
    }
    ...
    function on(y, P, U, Q, Ve, me) {
        ...
        if (Nt && !_t) return me || (me = new Rr), et || _n(y) ? Qi(y, P, U, Q, Ve, me) : ko(y, P, Ce, U, Q, Ve, me);
        if (!(U & o)) {
            ...
            if (tt || Yt) {
                var Cr = tt ? y.value() : y,
                    mr = Yt ? P.value() : P;
                return me || (me = new Rr), Ve(Cr, mr, U, Q, me)
            }
        }
        return Nt ? (me || (me = new Rr), un(y, P, U, Q, Ve, me)) : !1
    }

    function Ho(y) {
        if (!wn(y) || dn(y)) return !1;
        var P = Zi(y) ? Ht : Ke;
        return P.test(Wr(y))
    }
    ...
    function cn(y) {
        if (!gn(y)) return Oi(y);
        var P = [];
        for (var U in Object(y)) je.call(y, U) && U != "constructor" && P.push(U);
        return P
    }
    ...
    function Qi(y, P, U, Q, Ve, me) {
        var et = U & o,
            Tt = y.length,
            Ce = P.length;
        if (Tt != Ce && !(et && Ce > Tt)) return !1;
        var Ge = me.get(y);
        if (Ge && me.get(P)) return Ge == P;
        var _t = -1,
            ir = !0,
            Nt = U & h ? new Ji : void 0;
        for (me.set(y, P), me.set(P, y); ++_t < Tt;) {
            var tt = y[_t],
                Yt = P[_t];
            if (Q) var Cr = et ? Q(Yt, tt, _t, P, y, me) : Q(tt, Yt, _t, y, P, me);
            if (Cr !== void 0) {
                if (Cr) continue;
                ir = !1;
                break
            ...
            if (Nt) {
                if (!Se(P, function(mr, Hr) {
                        if (!ot(Nt, Hr) && (tt === mr || Ve(tt, mr, U, Q, me))) return Nt.push(Hr)
                    })) {
                    ir = !1;
                    break
                }
        ...
        return me.delete(y), me.delete(P), ir
    }

    function ko(y, P, U, Q, Ve, me, et) {
        switch (U) {
            case Te:
                if (y.byteLength != P.byteLength || y.byteOffset != P.byteOffset) return !1;
                y = y.buffer, P = P.buffer;
            case ue:
            ...
            case L:
                var Ce = Q & o;
                if (Tt || (Tt = at), y.size != P.size && !Ce) return !1;
                var Ge = et.get(y);
                if (Ge) return Ge == P;
                Q |= h, et.set(y, P);
                var _t = Qi(Tt(y), Tt(P), Q, Ve, me, et);
                return et.delete(y), _t;
            case W:
                if (yt) return yt.call(y) == yt.call(P)
        }
        return !1
    }
    ...
    function un(y, P, U, Q, Ve, me) {
        var et = U & o,
            ...
            Ge = Yi(P),
            _t = Ge.length;
        if (Ce != _t && !et) return !1;
        for (var ir = Ce; ir--;) {
            var Nt = Tt[ir];
            if (!(et ? Nt in P : je.call(P, Nt))) return !1
        }
        var tt = me.get(y);
        if (tt && me.get(P)) return tt == P;
        var Yt = !0;
        me.set(y, P), me.set(P, y);
        for (var Cr = et; ++ir < Ce;) {
            Nt = Tt[ir];
            var mr = y[Nt],
                Hr = P[Nt];
            if (Q) var xs = et ? Q(Hr, mr, Nt, P, y, me) : Q(mr, Hr, Nt, y, P, me);
        ...
        return me.delete(y), me.delete(P), Yt
    }
    ...
    function hn(y) {
        ...
        var Ve = $t.call(y);
        return Q && (P ? y[Qt] = U : delete y[Qt]), Ve
    }
    var ln = Vr ? function(y) {
            return y == null ? [] : (y = Object(y), Ne(Vr(y), function(P) {
                return rr.call(y, P)
            }))
        } : Fe,
        Ur = Ci;
    ...
    function fn(y, P) {
        return P = P ? ? p, !!P && (typeof y == "number" || jr.test(y)) && y > -1 && y % 1 == 0 && y < P
    }
...
function q_(a, r) {
    return r = r || {}, new Promise(function(i, n) {
        ...
        for (var v in o.open(r.method || "get", a, !0), o.onload = function() {
                o.getAllResponseHeaders().replace(/^(.*?):[^\S\n]*([\s\S]*?)$/gm, function(I, R, A) {
                    h.push(R = R.toLowerCase()), p.push([R, A]), g[R] = g[R] ? g[R] + "," + A : A
                }), i(_())
            }, o.onerror = n, o.withCredentials = r.credentials == "include", r.headers) o.setRequestHeader(v, r.headers[v]);
        o.send(r.body || null)
    })
}
...
function H_(a, r) {
    ...
    for (var o = 0; o < a.length; o++) {
        var h = a.charAt(o),
            p = h.charCodeAt(0);
        if (i[p] !== 255) throw new TypeError(h + " is ambiguous");
        i[p] = o
    }
    var g = a.length,
        _ = a.charAt(0),
    ...
    function R(T) {
        ...
        for (var F = 0, re = 0, he = 0, le = T.length; he !== le && T[he] === 0;) he++, F++;
        for (var fe = (le - he) * I + 1 >>> 0, pe = new Uint8Array(fe); he !== le;) {
            for (var de = T[he], L = 0, H = fe - 1;
                (de !== 0 || L < re) && H !== -1; H--, L++) de += 256 * pe[H] >>> 0, pe[H] = de % g >>> 0, de = de / g >>> 0;
            if (de !== 0) throw new Error("Non-zero carry");
            re = L, he++
        }
        for (var W = fe - re; W !== fe && pe[W] === 0;) W++;
        for (var ve = _.repeat(F); W < fe; ++W) ve += a.charAt(pe[W]);
        return ve
    }
    ...
    function A(T) {
        ...
        var F = 0;
        if (T[F] !== " ") {
            for (var re = 0, he = 0; T[F] === _;) re++, F++;
            for (var le = (T.length - F) * v + 1 >>> 0, fe = new Uint8Array(le); T[F];) {
                var pe = i[T.charCodeAt(F)];
                if (pe === 255) return;
                for (var de = 0, L = le - 1;
                    (pe !== 0 || de < he) && L !== -1; L--, de++) pe += g * fe[L] >>> 0, fe[L] = pe % 256 >>> 0, pe = pe / 256 >>> 0;
                if (pe !== 0) throw new Error("Non-zero carry");
                he = de, F++
            }
            if (T[F] !== " ") {
                for (var H = le - he; H !== le && fe[H] === 0;) H++;
                for (var W = new Uint8Array(re + (le - H)), ve = re; H !== le;) W[ve++] = fe[H++];
                return W
            }
        }
    }
...
class V_ {
    constructor(r, i, n) {
        this.name = r, this.prefix = i, this.baseEncode = n
    }
    encode(r) {
...
class G_ {
    constructor(r, i, n) {
        if (this.name = r, this.prefix = i, i.codePointAt(0) === void 0) throw new Error("Invalid prefix character");
...
const yf = (a, r) => new W_({ ...a.decoders || {
        [a.prefix]: a
    },
    ...r.decoders || {
        [r.prefix]: r
...
class J_ {
    constructor(r, i, n, o) {
        this.name = r, this.prefix = i, this.baseEncode = n, this.baseDecode = o, this.encoder = new V_(r, i, n), this.decoder = new G_(r, i, o)
    }
    encode(r) {
...
const xo = ({
    ...
    Q_ = (a, r, i, n) => {
        const o = {};
        for (let I = 0; I < r.length; ++I) o[r[I]] = I;
        let h = a.length;
        for (; a[h - 1] === "=";) --h;
        ...
        for (let I = 0; I < h; ++I) {
            ...
            _ = _ << i | R, g += i, g >= 8 && (g -= 8, p[v++] = 255 & _ >> g)
        }
        if (g >= i || 255 & _ << 8 - g) throw new SyntaxError("Unexpected end of data");
        return p
    },
    Y_ = (a, r, i) => {
        ...
        for (let _ = 0; _ < a.length; ++_)
            for (g = g << 8 | a[_], p += 8; p > i;) p -= i, h += r[o & g >> p];
        if (p && (h += r[o & g << i - p]), n)
            for (; h.length * i & 7;) h += "=";
        return h
    },
...
const vf = Array.from("🚀🪐☄🛰🌌🌑🌒🌓🌔🌕🌖🌗🌘🌍🌏🌎🐉☀💻🖥💾💿😂❤😍🤣😊🙏💕😭😘👍😅👏😁🔥🥰💔💖💙😢🤔😆🙄💪😉☺👌🤗💜😔😎😇🌹🤦🎉💞✌✨🤷😱😌🌸🙌😋💗💚😏💛🙂💓🤩😄😀🖤😃💯🙈👇🎶😒🤭❣😜💋👀😪😑💥🙋😞😩😡🤪👊🥳😥🤤👉💃😳✋😚😝😴🌟😬🙃🍀🌷😻😓⭐✅🥺🌈😈🤘💦✔😣🏃💐☹🎊💘😠☝😕🌺🎂🌻😐🖕💝🙊😹🗣💫💀👑🎵🤞😛🔴😤🌼😫⚽🤙☕🏆🤫👈😮🙆🍻🍃🐶💁😲🌿🧡🎁⚡🌞🎈❌✊👋😰🤨😶🤝🚶💰🍓💢🤟🙁🚨💨🤬✈🎀🍺🤓😙💟🌱😖👶🥴▶➡❓💎💸⬇😨🌚🦋😷🕺⚠🙅😟😵👎🤲🤠🤧📌🔵💅🧐🐾🍒😗🤑🌊🤯🐷☎💧😯💆👆🎤🙇🍑❄🌴💣🐸💌📍🥀🤢👅💡💩👐📸👻🤐🤮🎼🥵🚩🍎🍊👼💍📣🥂"),
    Ab = vf.reduce((a, r, i) => (a[i] = r, a), []),
    Tb = vf.reduce((a, r, i) => (a[r.codePointAt(0)] = i, a), []);

function Nb(a) {
    return a.reduce((r, i) => (r += Ab[i], r), "")
}
...
function mf(a, r, i) {
    r = r || [], i = i || 0;
    for (var n = i; a >= zb;) r[i++] = a & 255 | ul, a /= 128;
    for (; a & jb;) r[i++] = a & 255 | ul, a >>>= 7;
    return r[i] = a | 0, mf.bytes = i - n + 1, r
}
var Ub = yc,
    Hb = 128,
    hl = 127;
...
function yc(a, n) {
    var i = 0,
        ...
        p, g = a.length;
    do {
        if (h >= g) throw yc.bytes = 0, new RangeError("Could not decode varint");
        p = a[h++], i += o < 28 ? (p & hl) << o : (p & hl) * Math.pow(2, o), o += 7
    } while (p >= Hb);
    return yc.bytes = h - n, i
}
var kb = Math.pow(2, 7),
    Kb = Math.pow(2, 14),
    Bb = Math.pow(2, 21),
    ...
    wf = Xb;
const ll = (a, r, i = 0) => (wf.encode(a, r, i), r),
    ...
    vc = (a, r) => {
        ...
        return ll(a, h, 0), ll(i, h, n), h.set(r, o), new Zb(a, i, r, h)
    };
class Zb {
    constructor(r, i, n, o) {
        this.code = r, this.size = i, this.digest = n, this.bytes = o
    }
}
const _f = ({
    name: a,
...
class e1 {
    constructor(r, i, n) {
        this.name = r, this.code = i, this.encode = n
    }
    digest(r) {
...
const dl = Pf("utf8", "u", a => "u" + new TextDecoder("utf8").decode(a), a => new TextEncoder().encode(a.substring(1))),
    sc = Pf("ascii", "a", a => {
        let r = "a";
        for (let i = 0; i < a.length; i++) r += String.fromCharCode(a[i]);
        return r
    }, a => {
        a = a.substring(1);
        const r = c1(a.length);
        for (let i = 0; i < a.length; i++) r[i] = a.charCodeAt(i);
        return r
    }),
    u1 = {
        utf8: dl,
...
function h1(a, r = "utf8") {
    const i = u1[r];
    if (!i) throw new Error(`Unsupported encoding "${r}"`);
...
class k1 {
    constructor(r, i) {
        this.core = r, this.logger = i, this.keychain = new Map, this.name = y1, this.version = v1, this.initialized = !1, this.storagePrefix = xi, this.init = async () => {
            if (!this.initialized) {
            ...
            if (typeof o > "u") {
                const {
                    message: h
                } = Y("NO_MATCHING_KEY", `${this.name}: ${n}`);
                throw new Error(h)
            }
            return o
        }, this.del = async n => {
            this.isInitialized(), this.keychain.delete(n), await this.persist()
        }, this.core = r, this.logger = Re.generateChildLogger(i, this.name)
    }
    get context() {
...
class K1 {
    constructor(r, i, n) {
        this.core = r, this.logger = i, this.name = d1, this.initialized = !1, this.init = async () => {
            ...
            this.isInitialized();
            const p = h || w0(o);
            return await this.keychain.set(p, o), p
            ...
            try {
                ...
                this.logger.error(`Failed to decode message from topic: '${o}', clientId: '${await this.getClientId()}'`), this.logger.error(_)
            }
        ...
        }, this.core = r, this.logger = Re.generateChildLogger(i, this.name), this.keychain = n || new k1(this.core, this.logger)
    }
    get context() {
        return Re.getLoggerContext(this.logger)
    }
    async setPrivateKey(r, i) {
        return await this.keychain.set(r, i), r
    }
    getPrivateKey(r) {
    ...
    async getClientSeed() {
        ...
        try {
            ...
            r = pc(), await this.keychain.set(gl, r)
        }
        return h1(r, "base16")
    }
    getSymKey(r) {
...
class B1 extends Kw {
    constructor(r, i) {
        super(r, i), this.logger = r, this.core = i, this.messages = new Map, this.name = m1, this.version = w1, this.initialized = !1, this.storagePrefix = xi, this.init = async () => {
            if (!this.initialized) {
            ...
            return typeof p > "u" && (p = {}), typeof p[h] < "u" || (p[h] = o, this.messages.set(n, p), await this.persist()), h
        }, this.get = n => {
            this.isInitialized();
        ...
        }, this.logger = Re.generateChildLogger(r, this.name), this.core = i
    }
    get context() {
        return Re.getLoggerContext(this.logger)
    }
...
class F1 extends Bw {
    constructor(r, i) {
        super(r, i), this.relayer = r, this.logger = i, this.events = new Or.EventEmitter, this.name = b1, this.queue = new Map, this.publishTimeout = se.toMiliseconds(se.TEN_SECONDS), this.needsTransportRestart = !1, this.publish = async (n, o, h) => {
            var p;
            ...
            try {
                ...
                try {
                    await await Ys(this.rpcPublish(n, o, g, _, v, I, R), this.publishTimeout, "Failed to publish payload, please try again."), this.removeRequestFromQueue(R), this.relayer.events.emit(Wt.publish, A)
                } catch (T) {
                    if (this.logger.debug("Publishing Payload stalled"), this.needsTransportRestart = !0, (p = h == null ? void 0 : h.internal) != null && p.throwOnFailedPublish) throw this.removeRequestFromQueue(R), T;
                    return
                } finally {
                    clearTimeout(z)
                }
        ...
        }, this.relayer = r, this.logger = Re.generateChildLogger(i, this.name), this.registerEventListeners()
    }
    get context() {
        return Re.getLoggerContext(this.logger)
    }
    rpcPublish(r, i, n, o, h, p, g) {
        ...
        return Ii((_ = A.params) == null ? void 0 : _.prompt) && ((v = A.params) == null || delete v.prompt), Ii((I = A.params) == null ? void 0 : I.tag) && ((R = A.params) == null || delete R.tag), this.logger.debug("Outgoing Relay Payload"), this.logger.trace({
            type: "message",
            direction: "outgoing",
            request: A
...
class V1 {
    constructor() {
        this.map = new Map, this.set = (r, i) => {
            const n = this.get(r);
            this.exists(r, i) || this.map.set(r, [...n, i])
        }, this.get = r => this.map.get(r) || [], this.exists = (r, i) => this.get(r).includes(i), this.delete = (r, i) => {
            if (typeof i > "u") {
                this.map.delete(r);
                return
            }
...
var G1 = Object.defineProperty,
    ...
    Ks = (a, r) => {
        for (var i in r || (r = {})) Q1.call(r, i) && ml(a, i, r[i]);
        if (vl)
            for (var i of vl(r)) Y1.call(r, i) && ml(a, i, r[i]);
        return a
    },
    oc = (a, r) => W1(a, J1(r));
class X1 extends Gw {
    constructor(r, i) {
        super(r, i), this.relayer = r, this.logger = i, this.subscriptions = new Map, this.topicMap = new V1, this.events = new Or.EventEmitter, this.name = A1, this.version = T1, this.pending = new Map, this.cached = [], this.initialized = !1, this.pendingSubscriptionWatchLabel = "pending_sub_watch_label", this.pollingInterval = 20, this.storagePrefix = xi, this.subscribeTimeout = 1e4, this.restartInProgress = !1, this.batchSubscribeTopicsLimit = 500, this.init = async () => {
            ...
            try {
                ...
                return this.onSubscribe(g, p), this.logger.debug("Successfully Subscribed Topic"), this.logger.trace({
                    type: "method",
                    method: "subscribe",
                    params: {
                        topic: n,
            ...
            const g = setInterval(() => {
                !this.pending.has(n) && this.topics.includes(n) && (clearInterval(g), p.stop(this.pendingSubscriptionWatchLabel), o(!0)), p.elapsed(this.pendingSubscriptionWatchLabel) >= N1 && (clearInterval(g), p.stop(this.pendingSubscriptionWatchLabel), h(new Error("Subscription resolution timeout")))
        ...
        }, this.relayer = r, this.logger = Re.generateChildLogger(i, this.name), this.clientId = ""
    }
    get context() {
        return Re.getLoggerContext(this.logger)
    }
    ...
    async unsubscribeById(r, i, n) {
        ...
        try {
            ...
            await this.onUnsubscribe(r, i, h), this.logger.debug("Successfully Unsubscribed Topic"), this.logger.trace({
                type: "method",
                method: "unsubscribe",
                params: {
                    topic: r,
    ...
    onBatchSubscribe(r) {
        r.length && r.forEach(i => {
            this.setSubscription(i.id, Ks({}, i)), this.pending.delete(i.topic)
        })
    }
    async onUnsubscribe(r, i, n) {
        this.events.removeAllListeners(i), this.hasSubscription(i, r) && this.deleteSubscription(i, n), await this.relayer.messages.del(r)
    }
    ...
    addSubscription(r, i) {
        this.subscriptions.set(r, Ks({}, i)), this.topicMap.set(i.topic, r), this.events.emit($r.created, i)
    }
    getSubscription(r) {
    ...
    deleteSubscription(r, i) {
        ...
        this.subscriptions.delete(r), this.topicMap.delete(n.topic, r), this.events.emit($r.deleted, oc(Ks({}, n), {
            reason: i
        }))
    }
    async persist() {
    ...
    async restore() {
        try {
            ...
            if (this.subscriptions.size) {
                const {
                    message: i
                } = Y("RESTORE_WILL_OVERRIDE", this.name);
    ...
    async restartToComplete() {
        this.restartInProgress && await new Promise(r => {
            const i = setInterval(() => {
                this.restartInProgress || (clearInterval(i), r())
            }, this.pollingInterval)
        })
    }
}
var Z1 = Object.defineProperty,
    ...
    rE = (a, r) => {
        for (var i in r || (r = {})) eE.call(r, i) && _l(a, i, r[i]);
        if (wl)
            for (var i of wl(r)) tE.call(r, i) && _l(a, i, r[i]);
        return a
    };
class iE extends Fw {
    constructor(r) {
        super(r), this.protocol = "wc", this.version = 2, this.events = new Or.EventEmitter, this.name = I1, this.transportExplicitlyClosed = !1, this.initialized = !1, this.connectionAttemptInProgress = !1, this.connectionStatusPollingInterval = 20, this.staleConnectionErrors = ["socket hang up", "socket stalled"], this.hasExperiencedNetworkDisruption = !1, this.request = async i => {
            ...
            this.logger.error(i), this.events.emit(Wt.error, i), this.logger.info("Fatal socket error received, closing transport"), this.transportClose()
        }, this.registerProviderListeners = () => {
            this.provider.on(oi.payload, this.onPayloadHandler), this.provider.on(oi.connect, this.onConnectHandler), this.provider.on(oi.disconnect, this.onDisconnectHandler), this.provider.on(oi.error, this.onProviderErrorHandler)
        }, this.core = r.core, this.logger = typeof r.logger < "u" && typeof r.logger != "string" ? Re.generateChildLogger(r.logger, this.name) : Re.pino(Re.getDefaultLoggerOptions({
            level: r.logger || E1
        })), this.messages = new B1(this.logger, r.core), this.subscriber = new X1(this, this.logger), this.publisher = new F1(this, this.logger), this.relayUrl = (r == null ? void 0 : r.relayUrl) || Rf, this.projectId = r.projectId, this.bundleId = S0(), this.provider = {}
    }
    async init() {
        ...
        try {
            ...
            this.logger.warn(`Connection via ${this.relayUrl} failed, attempting to connect via failover domain ${yl}...`), await this.restartTransport(yl)
        }
    ...
    async publish(r, i, n) {
        this.isInitialized(), await this.publisher.publish(r, i, n), await this.recordMessageEvent({
            topic: r,
            message: i,
    ...
    async subscribe(r, i) {
        ...
        let h;
        const p = g => {
            g.topic === r && (this.subscriber.off($r.created, p), h())
        };
        return await Promise.all([new Promise(g => {
            h = g, this.subscriber.on($r.created, p)
        }), new Promise(async g => {
            o = await this.subscriber.subscribe(r, i), g()
        })]), o
    }
    async unsubscribe(r, i) {
        this.isInitialized(), await this.subscriber.unsubscribe(r, i)
    }
    on(r, i) {
        this.events.on(r, i)
    }
    ...
    async transportClose() {
        this.transportExplicitlyClosed = !0, this.hasExperiencedNetworkDisruption && this.connected ? await Ys(this.provider.disconnect(), 1e3, "provider.disconnect()").catch(() => this.onProviderDisconnect()) : this.connected && await this.provider.disconnect()
    }
    async transportOpen(r) {
        if (this.transportExplicitlyClosed = !1, await this.confirmOnlineStateOrThrow(), !this.connectionAttemptInProgress) {
            ...
            try {
                ...
                this.logger.error(i);
                const n = i;
                if (!this.isConnectionStalled(n.message)) throw i;
    ...
    async onProviderPayload(r) {
        if (this.logger.debug("Incoming Relay Payload"), this.logger.trace({
            ...
            if (!r.method.endsWith(x1)) return;
            const i = r.params,
                {
                    topic: n,
                    message: o,
            ...
            this.logger.debug("Emitting Relayer Payload"), this.logger.trace(rE({
                type: "event",
                event: i.id
            }, p)), this.events.emit(i.id, p), await this.acknowledgePayload(r), await this.onMessageEvent(p)
        } else Io(r) && this.events.emit(Wt.message_ack, r)
    }
    async onMessageEvent(r) {
        await this.shouldIgnoreMessageEvent(r) || (this.events.emit(Wt.message, r), await this.recordMessageEvent(r))
    }
    ...
    unregisterProviderListeners() {
        this.provider.off(oi.payload, this.onPayloadHandler), this.provider.off(oi.connect, this.onConnectHandler), this.provider.off(oi.disconnect, this.onDisconnectHandler), this.provider.off(oi.error, this.onProviderErrorHandler)
    }
    async registerEventListeners() {
        ...
        let r = await qh();
        R0(async i => {
            this.initialized && r !== i && (r = i, i ? await this.restartTransport().catch(n => this.logger.error(n)) : (this.hasExperiencedNetworkDisruption = !0, await this.transportClose().catch(n => this.logger.error(n))))
        })
    }
    ...
    async toEstablishConnection() {
        if (await this.confirmOnlineStateOrThrow(), !this.connected) {
            if (this.connectionAttemptInProgress) return await new Promise(r => {
                const i = setInterval(() => {
                    this.connected && (clearInterval(i), r())
...
var sE = Object.defineProperty,
    ...
    Il = (a, r) => {
        for (var i in r || (r = {})) nE.call(r, i) && El(a, i, r[i]);
        if (bl)
            for (var i of bl(r)) oE.call(r, i) && El(a, i, r[i]);
        return a
    };
class Po extends Vw {
    constructor(r, i, n, o = xi, h = void 0) {
        super(r, i, n, o), this.core = r, this.logger = i, this.name = n, this.map = new Map, this.version = R1, this.cached = [], this.initialized = !1, this.storagePrefix = xi, this.init = async () => {
            ...
            this.isInitialized(), this.map.has(p) ? await this.update(p, g) : (this.logger.debug("Setting value"), this.logger.trace({
                type: "method",
                method: "set",
                key: p,
                value: g
            ...
            method: "get",
            key: p
        }), this.getData(p)), this.getAll = p => (this.isInitialized(), p ? this.values.filter(g => Object.keys(p).every(_ => L_(g[_], p[_]))) : this.values), this.update = async (p, g) => {
            ...
            this.map.set(p, _), await this.persist()
        ...
        }, this.logger = Re.generateChildLogger(i, this.name), this.storagePrefix = o, this.getKey = h
    }
    get context() {
        return Re.getLoggerContext(this.logger)
    }
...
class aE {
    constructor(r, i) {
        this.core = r, this.logger = i, this.name = D1, this.version = $1, this.events = new Pc, this.initialized = !1, this.storagePrefix = xi, this.ignoredPayloadTypes = [Vl], this.registeredMethods = [], this.init = async () => {
            ...
            return await this.pairings.set(o, g), await this.core.relayer.subscribe(o), this.core.expirer.set(o, h), {
                topic: o,
                uri: _
            }
        }, this.pair = async n => {
            ...
            return await this.pairings.set(o, v), this.core.expirer.set(o, _), n.activatePairing && await this.activate({
                topic: o
            }), this.events.emit(Ws.create, v), this.core.crypto.keychain.has(o) || (await this.core.crypto.setSymKey(h, o), await this.core.relayer.subscribe(o, {
                relay: p
            })), v
        }, this.activate = async ({
            topic: n
            ...
            if (this.pairings.keys.includes(o)) {
                ...
                this.events.once(Dt("pairing_ping", h), ({
                    error: v
                }) => {
                    v ? _(v) : g()
                }), await p()
            ...
            this.pairings.keys.includes(o) && (await this.sendRequest(o, "wc_pairingDelete", Gt("USER_DISCONNECTED")), await this.deletePairing(o))
        }, this.sendRequest = async (n, o, h) => {
            const p = vs(o, h),
                g = await this.core.crypto.encode(n, p),
                _ = ks[o].req;
            return this.core.history.set(n, p), this.core.relayer.publish(n, g, _), p.id
        }, this.sendResult = async (n, o, h) => {
            const p = bo(n, h),
                g = await this.core.crypto.encode(o, p),
                _ = await this.core.history.get(o, n),
                v = ks[_.request.method].res;
            await this.core.relayer.publish(o, g, v), await this.core.history.resolve(p)
            ...
            await this.core.relayer.unsubscribe(n), await Promise.all([this.pairings.delete(n, Gt("USER_DISCONNECTED")), this.core.crypto.deleteSymKey(n), o ? Promise.resolve() : this.core.expirer.del(n)])
        }, this.cleanup = async () => {
            ...
            const {
                topic: o,
                payload: h
            } = n, p = (await this.core.history.get(o, h.id)).request.method;
            switch (p) {
                case "wc_pairingPing":
            ...
            try {
                this.isValidPing({
                    topic: n
                }), await this.sendResult(h, n, !0), this.events.emit(Ws.ping, {
                    id: h,
                    topic: n
                })
            } catch (p) {
                await this.sendError(h, n, p), this.logger.error(p)
            }
            ...
            try {
                this.isValidDisconnect({
                    topic: n
                }), await this.deletePairing(n), this.events.emit(Ws.delete, {
                    id: h,
                    topic: n
                })
            } catch (p) {
            ...
            if (!hr(n)) {
                const {
                    message: p
                } = Y("MISSING_OR_INVALID", `pair() params: ${n}`);
                throw new Error(p)
            }
            if (!A0(n.uri)) {
                const {
                    message: p
                } = Y("MISSING_OR_INVALID", `pair() uri: ${n.uri}`);
                throw new Error(p)
            }
            const h = Mh(n.uri);
            if (!((o = h == null ? void 0 : h.relay) != null && o.protocol)) {
                const {
                    message: p
                } = Y("MISSING_OR_INVALID", "pair() uri#relay-protocol");
                throw new Error(p)
            }
            if (!(h != null && h.symKey)) {
                const {
                    message: p
                } = Y("MISSING_OR_INVALID", "pair() uri#symKey");
                throw new Error(p)
            }
            ...
            if (!hr(n)) {
                const {
                    message: h
                } = Y("MISSING_OR_INVALID", `ping() params: ${n}`);
                throw new Error(h)
            }
            const {
                topic: o
            ...
            if (!hr(n)) {
                const {
                    message: h
                } = Y("MISSING_OR_INVALID", `disconnect() params: ${n}`);
                throw new Error(h)
            }
            const {
                topic: o
        ...
        }, this.core = r, this.logger = Re.generateChildLogger(i, this.name), this.pairings = new Po(this.core, this.logger, this.name, this.storagePrefix)
    }
    get context() {
        return Re.getLoggerContext(this.logger)
    }
    ...
    registerRelayerEvents() {
        this.core.relayer.on(Wt.message, async r => {
            ...
            if (!this.pairings.keys.includes(i) || this.ignoredPayloadTypes.includes(this.core.crypto.getPayloadType(n))) return;
            const o = await this.core.crypto.decode(i, n);
            try {
                Cc(o) ? (this.core.history.set(i, o), this.onRelayEventRequest({
                    topic: i,
                    payload: o
                })) : Io(o) && (await this.core.history.resolve(o), await this.onRelayEventResponse({
                    topic: i,
                    payload: o
    ...
    registerExpirerEvents() {
        this.core.expirer.on(Sr.expired, async r => {
            ...
            i && this.pairings.keys.includes(i) && (await this.deletePairing(i, !0), this.events.emit(Ws.expire, {
                topic: i
            }))
        })
    }
...
class cE extends kw {
    constructor(r, i) {
        super(r, i), this.core = r, this.logger = i, this.records = new Map, this.events = new Or.EventEmitter, this.name = L1, this.version = q1, this.cached = [], this.initialized = !1, this.storagePrefix = xi, this.init = async () => {
            ...
            this.records.set(p.id, p), this.events.emit(Fr.created, p)
            ...
            typeof o.response > "u" && (o.response = qr(n) ? {
                ...
                result: n.result
            }, this.records.set(o.id, o), this.events.emit(Fr.updated, o))
            ...
            this.isInitialized(), this.logger.debug("Deleting record"), this.logger.trace({
                ...
                if (h.topic === n) {
                    ...
                    this.records.delete(h.id), this.events.emit(Fr.deleted, h)
                }
            })
    ...
    cleanup() {
        try {
            this.records.forEach(r => {
                se.toMiliseconds(r.expiry || 0) - Date.now() <= 0 && (this.logger.info(`Deleting expired history log: ${r.id}`), this.delete(r.topic, r.id))
            })
        } catch (r) {
            this.logger.warn(r)
        }
...
class uE extends Ww {
    constructor(r, i) {
        super(r, i), this.core = r, this.logger = i, this.expirations = new Map, this.events = new Or.EventEmitter, this.name = M1, this.version = j1, this.cached = [], this.initialized = !1, this.storagePrefix = xi, this.init = async () => {
            ...
            this.expirations.set(h, p), this.checkExpiry(h, p), this.events.emit(Sr.created, {
                target: h,
                expiration: p
            })
        }, this.get = n => {
            ...
            if (this.isInitialized(), this.has(n)) {
                ...
                this.expirations.delete(o), this.events.emit(Sr.deleted, {
                    target: o,
                    expiration: h
                })
            }
    ...
    formatTarget(r) {
        if (typeof r == "string") return T0(r);
        if (typeof r == "number") return N0(r);
        const {
            message: i
        } = Y("UNKNOWN_TYPE", `Target type: ${typeof r}`);
        throw new Error(i)
    }
    ...
    expire(r, i) {
        this.expirations.delete(r), this.events.emit(Sr.expired, {
            target: r,
            expiration: i
        })
    }
    ...
    registerEventListeners() {
        this.core.heartbeat.on(_s.HEARTBEAT_EVENTS.pulse, () => this.checkExpirations()), this.events.on(Sr.created, r => {
            const i = Sr.created;
            this.logger.info(`Emitting ${i}`), this.logger.debug({
                type: "event",
                event: i,
...
class hE extends Jw {
    constructor(r, i) {
        super(r, i), this.projectId = r, this.logger = i, this.name = nc, this.initialized = !1, this.queue = [], this.verifyDisabled = !1, this.init = async n => {
            if (this.verifyDisabled || D0() || !Wl()) return;
            ...
            if (!this.initialized) {
                this.removeIframe(), this.verifyUrl = mc;
                try {
                    await this.createIframe()
                } catch (h) {
            ...
            try {
                ...
                this.logger.info(`failed to resolve attestation: ${n.attestationId} from url: ${o}`), this.logger.info(p), h = await this.fetchAttestation(n.attestationId, mc)
            }
            return h
            ...
            let n;
            const o = h => {
                h.data === "verify_ready" && (this.initialized = !0, this.processQueue(), window.removeEventListener("message", o), n())
            };
            await Promise.race([new Promise(h => {
                if (document.getElementById(nc)) return h();
                window.addEventListener("message", o);
                const p = document.createElement("iframe");
                p.id = nc, p.src = `${this.verifyUrl}/${this.projectId}`, p.style.display = "none", document.body.append(p), this.iframe = p, n = h
            }), new Promise((h, p) => setTimeout(() => {
                window.removeEventListener("message", o), p("verify iframe load timeout")
            }, se.toMiliseconds(se.FIVE_SECONDS)))])
        }, this.removeIframe = () => {
            this.iframe && (this.iframe.remove(), this.iframe = void 0, this.initialized = !1)
        }, this.getVerifyUrl = n => {
            let o = n || gs;
            return z1.includes(o) || (this.logger.info(`verify url: ${o}, not included in trusted list, assigning default: ${gs}`), o = gs), o
        }, this.logger = Re.generateChildLogger(i, this.name), this.verifyUrl = gs, this.abortController = new AbortController, this.isDevEnv = $0() && U_.IS_VITEST
    }
    get context() {
        return Re.getLoggerContext(this.logger)
    }
    startAbortTimer(r) {
        return this.abortController = new AbortController, setTimeout(() => this.abortController.abort(), se.toMiliseconds(r))
    }
}
class lE extends Qw {
    constructor(r, i) {
        super(r, i), this.projectId = r, this.logger = i, this.context = U1, this.registerDeviceToken = async n => {
            const {
                clientId: o,
                token: h,
                notificationType: p,
                enableEncrypted: g = !1
            } = n, _ = `${H1}/${this.projectId}/clients`;
            await z_(_, {
                method: "POST",
                headers: {
...
var fE = Object.defineProperty,
    ...
    Sl = (a, r) => {
        for (var i in r || (r = {})) pE.call(r, i) && Pl(a, i, r[i]);
        if (xl)
            for (var i of xl(r)) dE.call(r, i) && Pl(a, i, r[i]);
        return a
    };
class Tc extends Hw {
    constructor(r) {
        super(r), this.protocol = Sf, this.version = l1, this.name = Ac, this.events = new Or.EventEmitter, this.initialized = !1, this.on = (n, o) => this.events.on(n, o), this.once = (n, o) => this.events.once(n, o), this.off = (n, o) => this.events.off(n, o), this.removeListener = (n, o) => this.events.removeListener(n, o), this.projectId = r == null ? void 0 : r.projectId, this.relayUrl = (r == null ? void 0 : r.relayUrl) || Rf, this.customStoragePrefix = r != null && r.customStoragePrefix ? `:${r.customStoragePrefix}` : "";
        ...
        this.logger = Re.generateChildLogger(i, this.name), this.heartbeat = new _s.HeartBeat, this.crypto = new K1(this, this.logger, r == null ? void 0 : r.keychain), this.history = new cE(this, this.logger), this.expirer = new uE(this, this.logger), this.storage = r != null && r.storage ? r.storage : new Nw(Sl(Sl({}, p1), r == null ? void 0 : r.storageOptions)), this.relayer = new iE({
            core: this,
            logger: this.logger,
            relayUrl: this.relayUrl,
            projectId: this.projectId
        }), this.pairing = new aE(this, this.logger), this.verify = new hE(this.projectId || "", this.logger), this.echoClient = new lE(this.projectId || "", this.logger)
    }
    static async init(r) {
        const i = new Tc(r);
        await i.initialize();
        const n = await i.crypto.getClientId();
        return await i.storage.setItem(C1, n), i
    }
    get context() {
        return Re.getLoggerContext(this.logger)
    }
...
var bE = Object.defineProperty,
    ...
    cr = (a, r) => {
        for (var i in r || (r = {})) xE.call(r, i) && Cl(a, i, r[i]);
        if (Rl)
            for (var i of Rl(r)) PE.call(r, i) && Cl(a, i, r[i]);
        return a
    },
    Fs = (a, r) => EE(a, IE(r));
class SE extends Xw {
    constructor(r) {
        super(r), this.name = mE, this.events = new Pc, this.initialized = !1, this.ignoredPayloadTypes = [Vl], this.requestQueue = {
            state: ai.idle,
            queue: []
        }, this.sessionRequestQueue = {
            state: ai.idle,
            queue: []
        }, this.requestQueueDelay = se.ONE_SECOND, this.init = async () => {
            ...
            let v = o,
                I, R = !1;
            if (v && (R = this.client.core.pairing.pairings.get(v).active), !v || !R) {
                const {
                    topic: fe,
                    uri: pe
            ...
            const A = await this.client.core.crypto.generateKeyPair(),
                ...
                } = ps(se.FIVE_MINUTES, Nf);
            if (this.events.once(Dt("session_connect"), async ({
                    error: fe,
                    session: pe
                }) => {
                    if (fe) T(fe);
                        ...
                        await this.client.session.set(pe.topic, de), await this.setExpiry(pe.topic, pe.expiry), v && await this.client.core.pairing.updateMetadata({
                            topic: v,
                ...
                const {
                    message: fe
                } = Y("NO_MATCHING_KEY", `connect() pairing topic: ${v}`);
                throw new Error(fe)
            }
        ...
        }, this.pair = async i => (await this.isInitialized(), await this.client.core.pairing.pair(i)), this.approve = async i => {
            await this.isInitialized(), await this.isValidApprove(i);
            const {
                id: n,
                relayProtocol: o,
                namespaces: h,
                sessionProperties: p
            } = i, g = this.client.proposal.get(n);
            let {
                pairingTopic: _,
                proposer: v,
            ...
            }), await this.client.proposal.delete(n, Gt("USER_DISCONNECTED")), await this.client.core.pairing.activate({
                topic: _
            }));
            const F = cr({
                relay: {
            ...
            return await this.client.session.set(T, re), await this.setExpiry(T, Lr(uo)), {
                topic: T,
                acknowledged: () => new Promise(he => setTimeout(() => he(this.client.session.get(T)), 500))
            }
        }, this.reject = async i => {
            await this.isInitialized(), await this.isValidReject(i);
            const {
                id: n,
                reason: o
            } = i, {
            ...
            h && (await this.sendError(n, h, o), await this.client.proposal.delete(n, Gt("USER_DISCONNECTED")))
        }, this.update = async i => {
            await this.isInitialized(), await this.isValidUpdate(i);
            const {
                topic: n,
                namespaces: o
            } = i, h = await this.sendRequest({
                topic: n,
            ...
            return this.events.once(Dt("session_update", h), ({
                error: v
            }) => {
                v ? _(v) : g()
            ...
            await this.isInitialized(), await this.isValidExtend(i);
            const {
                topic: n
            } = i, o = await this.sendRequest({
                topic: n,
            ...
            return this.events.once(Dt("session_extend", o), ({
                error: _
            }) => {
                _ ? g(_) : p()
            ...
            await this.isInitialized(), await this.isValidRequest(i);
            const {
                chainId: n,
                request: o,
                topic: h,
                ...
                resolve: v,
                reject: I
            } = ps(p, "Request expired. Please try again.");
            return this.events.once(Dt("session_request", g), ({
                error: R,
                result: A
            }) => {
                R ? I(R) : v(A)
                ...
                await this.sendRequest({
                    ...
                    expiry: p,
                    throwOnFailedPublish: !0
                }).catch(A => I(A)), this.client.events.emit("session_request_sent", {
                    topic: h,
                    request: o,
                    chainId: n,
                    id: g
            ...
            await this.isInitialized(), await this.isValidRespond(i);
            const {
                topic: n,
                response: o
            } = i, {
            ...
            ci(o) ? await this.sendResult({
                ...
                result: o.result,
                throwOnFailedPublish: !0
            }) : qr(o) && await this.sendError(h, n, o.error), this.cleanupAfterResponse(i)
        }, this.ping = async i => {
            await this.isInitialized(), await this.isValidPing(i);
            const {
                topic: n
            } = i;
            if (this.client.session.keys.includes(n)) {
                ...
                this.events.once(Dt("session_ping", o), ({
                    error: _
                }) => {
                    _ ? g(_) : p()
                }), await h()
            ...
            await this.isInitialized(), await this.isValidEmit(i);
            const {
                topic: n,
                event: o,
                chainId: h
            ...
            await this.isInitialized(), await this.isValidDisconnect(i);
            const {
                topic: n
            } = i;
        ...
        }, this.find = i => (this.isInitialized(), this.client.session.getAll().filter(n => z0(n, i))), this.getPendingSessionRequests = () => (this.isInitialized(), this.client.pendingRequest.getAll()), this.cleanupDuplicatePairings = async i => {
            if (i.pairingTopic) try {
                ...
                this.client.logger.info(`Cleaning up ${o.length} duplicate pairing(s)`), await Promise.all(o.map(h => this.client.core.pairing.disconnect({
                    topic: h.topic
            ...
            await Promise.all([this.client.pendingRequest.delete(i, n), o ? Promise.resolve() : this.client.core.expirer.del(i)]), this.sessionRequestQueue.queue = this.sessionRequestQueue.queue.filter(h => h.id !== i), o && (this.sessionRequestQueue.state = ai.idle)
            ...
            await this.client.proposal.set(i, n), this.client.core.expirer.set(i, n.expiry)
            ...
            const R = await this.client.core.crypto.encode(n, I),
                A = Bs[o].req;
            return p && (A.ttl = p), g && (A.id = g), this.client.core.history.set(n, I), v ? (A.internal = Fs(cr({}, A.internal), {
                throwOnFailedPublish: !0
            }), await this.client.core.relayer.publish(n, R, A)) : this.client.core.relayer.publish(n, R, A).catch(z => this.client.logger.error(z)), I.id
            ...
            } = i, g = bo(n, h), _ = await this.client.core.crypto.encode(o, g), v = await this.client.core.history.get(o, n), I = Bs[v.request.method].res;
            p ? (I.internal = Fs(cr({}, I.internal), {
                throwOnFailedPublish: !0
            }), await this.client.core.relayer.publish(o, _, I)) : this.client.core.relayer.publish(o, _, I).catch(R => this.client.logger.error(R)), await this.client.core.history.resolve(g)
            ...
            this.client.core.relayer.publish(n, p, _), await this.client.core.history.resolve(h)
        }, this.cleanup = async () => {
            const i = [],
                n = [];
            ...
            for (this.client.logger.info(`Request queue starting with ${this.requestQueue.queue.length} requests`); this.requestQueue.queue.length > 0;) {
                ...
                if (i) try {
                    this.processRequest(i), await new Promise(n => setTimeout(n, 300))
                } catch (n) {
            ...
            const {
                topic: n,
                payload: o
            } = i, h = (await this.client.core.history.get(n, o.id)).request.method;
            switch (h) {
                case "wc_sessionPropose":
            ...
            const {
                ...
                message: o
            } = Y("MISSING_OR_INVALID", `Decoded payload on topic ${n} is not identifiable as a JSON-RPC request or a response.`);
            throw new Error(o)
            ...
            try {
                ...
                await this.sendError(h, i, p), this.client.logger.error(p)
            }
            ...
            } else qr(n) && (await this.client.proposal.delete(o, Gt("USER_DISCONNECTED")), this.events.emit(Dt("session_connect"), {
                error: n.error
            }))
            ...
            try {
                ...
                await this.sendError(o, i, p), this.client.logger.error(p)
            }
            ...
            ci(n) ? (await this.client.session.update(i, {
                acknowledged: !0
            }), this.events.emit(Dt("session_approve", o), {})) : qr(n) && (await this.client.session.delete(i, Gt("USER_DISCONNECTED")), this.events.emit(Dt("session_approve", o), {
                error: n.error
            }))
            ...
            try {
                ...
                this.isValidUpdate(cr({
                    topic: i
                }, o)), await this.client.session.update(i, {
        ...
        }, this.isRequestOutOfSync = (i, n) => parseInt(n.toString().slice(0, -3)) <= parseInt(i.toString().slice(0, -3)), this.onSessionUpdateResponse = (i, n) => {
            const {
                id: o
            } = n;
            ...
            try {
                this.isValidExtend({
                    topic: i
                }), await this.setExpiry(i, Lr(uo)), await this.sendResult({
                    id: o,
                    topic: i,
                    result: !0
                ...
                await this.sendError(o, i, h), this.client.logger.error(h)
            }
            ...
            try {
                ...
                await this.setPendingSessionRequest(v), this.addSessionRequestToSessionRequestQueue(v), this.processSessionRequestQueue()
            } catch (p) {
                await this.sendError(o, i, p), this.client.logger.error(p)
            }
            ...
            try {
                ...
                this.isValidEmit(cr({
                    topic: i
                }, h)), this.client.events.emit("session_event", {
                    id: o,
                    topic: i,
                    params: h
                }), oo.set(p, o)
            ...
            this.deletePendingSessionRequest(i.response.id, {
                ...
                this.sessionRequestQueue.state = ai.idle, this.processSessionRequestQueue()
            }, se.toMiliseconds(this.requestQueueDelay))
            ...
            try {
                this.sessionRequestQueue.state = ai.active, this.client.events.emit("session_request", i)
            } catch (n) {
                this.client.logger.error(n)
            }
            ...
            if (!hr(i)) {
                const {
                    message: _
                } = Y("MISSING_OR_INVALID", `connect() params: ${JSON.stringify(i)}`);
                throw new Error(_)
            }
            const {
                pairingTopic: n,
            ...
            if (Ii(n) || await this.isValidPairingTopic(n), !U0(g, !0)) {
                const {
                    message: _
                } = Y("MISSING_OR_INVALID", `connect() relays: ${g}`);
                throw new Error(_)
            }!Ii(o) && po(o) !== 0 && this.validateNamespaces(o, "requiredNamespaces"), !Ii(h) && po(h) !== 0 && this.validateNamespaces(h, "optionalNamespaces"), Ii(p) || this.validateSessionProps(p, "sessionProperties")
        }, this.validateNamespaces = (i, n) => {
            const o = H0(i, "connect()", n);
            if (o) throw new Error(o.message)
        }, this.isValidApprove = async i => {
            if (!hr(i)) throw new Error(Y("MISSING_OR_INVALID", `approve() params: ${i}`).message);
            const {
                id: n,
                namespaces: o,
                relayProtocol: h,
            ...
            if (!ds(h, !0)) {
                const {
                    message: I
                } = Y("MISSING_OR_INVALID", `approve() relayProtocol: ${h}`);
                throw new Error(I)
            }
            ...
            if (!hr(i)) {
                const {
                    message: h
                } = Y("MISSING_OR_INVALID", `reject() params: ${i}`);
                throw new Error(h)
            }
            const {
                id: n,
            ...
            if (await this.isValidProposalId(n), !k0(o)) {
                const {
                    message: h
                } = Y("MISSING_OR_INVALID", `reject() reason: ${JSON.stringify(o)}`);
                throw new Error(h)
            }
            ...
            if (!hr(i)) {
                const {
                    message: v
                } = Y("MISSING_OR_INVALID", `onSessionSettleRequest() params: ${i}`);
                throw new Error(v)
            }
            const {
                relay: n,
            ...
            if (!hr(i)) {
                const {
                    message: _
                } = Y("MISSING_OR_INVALID", `update() params: ${i}`);
                throw new Error(_)
            }
            const {
                topic: n,
            ...
            if (!hr(i)) {
                const {
                    message: o
                } = Y("MISSING_OR_INVALID", `extend() params: ${i}`);
                throw new Error(o)
            }
            const {
                topic: n
            ...
            if (!hr(i)) {
                const {
                    message: _
                } = Y("MISSING_OR_INVALID", `request() params: ${i}`);
                throw new Error(_)
            }
            const {
                topic: n,
            ...
            if (!zh(g, h)) {
                const {
                    message: _
                } = Y("MISSING_OR_INVALID", `request() chainId: ${h}`);
                throw new Error(_)
            }
            if (!F0(o)) {
                const {
                    message: _
                } = Y("MISSING_OR_INVALID", `request() ${JSON.stringify(o)}`);
                throw new Error(_)
            }
            if (!V0(g, h, o.method)) {
                const {
                    message: _
                } = Y("MISSING_OR_INVALID", `request() method: ${o.method}`);
                throw new Error(_)
            }
            if (p && !G0(p, cc)) {
                const {
                    message: _
                } = Y("MISSING_OR_INVALID", `request() expiry: ${p}. Expiry must be a number (in seconds) between ${cc.min} and ${cc.max}`);
                throw new Error(_)
            }
            ...
            if (!hr(i)) {
                const {
                    message: p
                } = Y("MISSING_OR_INVALID", `respond() params: ${i}`);
                throw new Error(p)
            }
            const {
                topic: o,
            ...
            try {
                ...
                throw (n = i == null ? void 0 : i.response) != null && n.id && this.cleanupAfterResponse(i), p
            }
            if (!W0(h)) {
                const {
                    message: p
                } = Y("MISSING_OR_INVALID", `respond() response: ${JSON.stringify(h)}`);
                throw new Error(p)
            }
            ...
            if (!hr(i)) {
                const {
                    message: o
                } = Y("MISSING_OR_INVALID", `ping() params: ${i}`);
                throw new Error(o)
            }
            const {
                topic: n
            ...
            if (!hr(i)) {
                const {
                    message: g
                } = Y("MISSING_OR_INVALID", `emit() params: ${i}`);
                throw new Error(g)
            }
            const {
                topic: n,
            ...
            if (!zh(p, h)) {
                const {
                    message: g
                } = Y("MISSING_OR_INVALID", `emit() chainId: ${h}`);
                throw new Error(g)
            }
            if (!J0(o)) {
                const {
                    message: g
                } = Y("MISSING_OR_INVALID", `emit() event: ${JSON.stringify(o)}`);
                throw new Error(g)
            }
            if (!Q0(p, h, o.name)) {
                const {
            ...
            if (!hr(i)) {
                const {
                    message: o
                } = Y("MISSING_OR_INVALID", `disconnect() params: ${i}`);
                throw new Error(o)
            }
            const {
                topic: n
            ...
            try {
                ...
                h && (o.verified.origin = h.origin, o.verified.isScam = h.isScam, o.verified.validation = h.origin === new URL(n.url).origin ? "VALID" : "INVALID")
            } catch (h) {
            ...
            Object.values(i).forEach(o => {
                if (!ds(o, !1)) {
                    const {
                        message: h
                    } = Y("MISSING_OR_INVALID", `${n} must be in Record<string, string> format. Received: ${JSON.stringify(o)}`);
                    throw new Error(h)
                }
            })
        }
    ...
    registerRelayerEvents() {
        this.client.core.relayer.on(Wt.message, async r => {
            ...
            try {
                Cc(o) ? (this.client.core.history.set(i, o), this.onRelayEventRequest({
                    topic: i,
                    payload: o
                })) : Io(o) ? (await this.client.core.history.resolve(o), await this.onRelayEventResponse({
                    topic: i,
                    payload: o
    ...
    registerExpirerEvents() {
        this.client.core.expirer.on(Sr.expired, async r => {
            ...
            if (n && this.client.pendingRequest.keys.includes(n)) return await this.deletePendingSessionRequest(n, Y("EXPIRED"), !0);
    ...
    isValidPairingTopic(r) {
        if (!ds(r, !1)) {
            const {
                message: i
            } = Y("MISSING_OR_INVALID", `pairing topic should be a string: ${r}`);
            throw new Error(i)
        }
        if (!this.client.core.pairing.pairings.keys.includes(r)) {
            const {
                message: i
            } = Y("NO_MATCHING_KEY", `pairing topic doesn't exist: ${r}`);
            throw new Error(i)
        }
        if (Ei(this.client.core.pairing.pairings.get(r).expiry)) {
            const {
                message: i
            } = Y("EXPIRED", `pairing topic: ${r}`);
            throw new Error(i)
        }
    }
    async isValidSessionTopic(r) {
        if (!ds(r, !1)) {
            const {
                message: i
            } = Y("MISSING_OR_INVALID", `session topic should be a string: ${r}`);
            throw new Error(i)
        }
        if (!this.client.session.keys.includes(r)) {
            const {
                message: i
            } = Y("NO_MATCHING_KEY", `session topic doesn't exist: ${r}`);
            throw new Error(i)
        }
    ...
    async isValidSessionOrPairingTopic(r) {
        if (this.client.session.keys.includes(r)) await this.isValidSessionTopic(r);
            ...
            const {
                message: i
            } = Y("NO_MATCHING_KEY", `session or pairing topic doesn't exist: ${r}`);
            throw new Error(i)
        } else {
            const {
                message: i
            } = Y("MISSING_OR_INVALID", `session or pairing topic should be a string: ${r}`);
            throw new Error(i)
        }
    }
    async isValidProposalId(r) {
        if (!Y0(r)) {
            const {
                message: i
            } = Y("MISSING_OR_INVALID", `proposal id should be a number: ${r}`);
            throw new Error(i)
        }
        if (!this.client.proposal.keys.includes(r)) {
            const {
                message: i
            } = Y("NO_MATCHING_KEY", `proposal id doesn't exist: ${r}`);
            throw new Error(i)
        }
...
class OE extends Po {
    constructor(r, i) {
        super(r, i, yE, Nc), this.core = r, this.logger = i
    }
}
class RE extends Po {
    constructor(r, i) {
        super(r, i, vE, Nc), this.core = r, this.logger = i
    }
}
class CE extends Po {
    constructor(r, i) {
        super(r, i, wE, Nc, n => n.id), this.core = r, this.logger = i
    }
}
let AE = class Df extends Yw {
    constructor(r) {
        super(r), this.protocol = Cf, this.version = Af, this.name = ac.name, this.events = new Or.EventEmitter, this.on = (n, o) => this.events.on(n, o), this.once = (n, o) => this.events.once(n, o), this.off = (n, o) => this.events.off(n, o), this.removeListener = (n, o) => this.events.removeListener(n, o), this.removeAllListeners = n => this.events.removeAllListeners(n), this.connect = async n => {
            try {
        ...
        }, this.name = (r == null ? void 0 : r.name) || ac.name, this.metadata = (r == null ? void 0 : r.metadata) || L0();
        const i = typeof(r == null ? void 0 : r.logger) < "u" && typeof(r == null ? void 0 : r.logger) != "string" ? r.logger : Re.pino(Re.getDefaultLoggerOptions({
            level: (r == null ? void 0 : r.logger) || ac.logger
        }));
        this.core = (r == null ? void 0 : r.core) || new gE(r), this.logger = Re.generateChildLogger(i, this.name), this.session = new RE(this.core, this.logger), this.proposal = new OE(this.core, this.logger), this.pendingRequest = new CE(this.core, this.logger), this.engine = new SE(this)
    }
    static async init(r) {
        const i = new Df(r);
...
(function(a, r) {
    ...
    (function(h) {
        (function(p) {
            ...
            function T(m) {
                this.map = {}, m instanceof T ? m.forEach(function(x, X) {
                    this.append(X, x)
            ...
            function fe(m) {
                for (var x = new Uint8Array(m), X = new Array(x.length), ce = 0; ce < x.length; ce++) X[ce] = String.fromCharCode(x[ce]);
                return X.join("")
            }
            ...
            function W(m, x) {
                ...
                if (m instanceof W) {
                    ...
                    this.url = m.url, this.credentials = m.credentials, x.headers || (this.headers = new T(m.headers)), this.method = m.method, this.mode = m.mode, this.signal = m.signal, !X && m._bodyInit != null && (X = m._bodyInit, m.bodyUsed = !0)
                } else this.url = String(m);
                if (this.credentials = x.credentials || this.credentials || "same-origin", (x.headers || !this.headers) && (this.headers = new T(x.headers)), this.method = H(x.method || this.method || "GET"), this.mode = x.mode || this.mode || null, this.signal = x.signal || this.signal, this.referrer = null, (this.method === "GET" || this.method === "HEAD") && X) throw new TypeError("Body not allowed for GET or HEAD requests");
                this._initBody(X)
            }
            ...
            function ue(m, x) {
                x || (x = {}), this.type = "default", this.status = x.status === void 0 ? 200 : x.status, this.ok = this.status >= 200 && this.status < 300, this.statusText = "statusText" in x ? x.statusText : "OK", this.headers = new T(x.headers), this.url = x.url || "", this._initBody(m)
            }
            ...
            function Ue(m, x) {
                return new Promise(function(X, ce) {
                    var V = new W(m, x);
                    if (V.signal && V.signal.aborted) return ce(new p.DOMException("Aborted", "AbortError"));
                    ...
                    }, k.open(V.method, V.url, !0), V.credentials === "include" ? k.withCredentials = !0 : V.credentials === "omit" && (k.withCredentials = !1), "responseType" in k && g.blob && (k.responseType = "blob"), V.headers.forEach(function(G, Je) {
                        k.setRequestHeader(Je, G)
                    }), V.signal && (V.signal.addEventListener("abort", B), k.onreadystatechange = function() {
                        k.readyState === 4 && V.signal.removeEventListener("abort", B)
                    }), k.send(typeof V._bodyInit > "u" ? null : V._bodyInit)
                })
            }
            return Ue.polyfill = !0, h.fetch || (h.fetch = Ue, h.Headers = T, h.Request = W, h.Response = ue), p.Headers = T, p.Request = W, p.Response = ue, p.fetch = Ue, Object.defineProperty(p, "__esModule", {
                value: !0
            }), p
        })({})
    })(n), n.fetch.ponyfill = !0, delete n.fetch.polyfill;
    var o = n;
    r = o.fetch, r.default = o.fetch, r.fetch = o.fetch, r.Headers = o.Headers, r.Request = o.Request, r.Response = o.Response, a.exports = r
})(wc, wc.exports);
var TE = wc.exports;
const Al = xc(TE),
    NE = {
...
class Si {
    constructor(r, i = !1) {
        if (this.url = r, this.disableProviderPing = i, this.events = new Or.EventEmitter, this.isAvailable = !1, this.registering = !1, !sl(r)) throw new Error(`Provided URL is not compatible with HTTP connection: ${r}`);
        this.url = r, this.disableProviderPing = i
    }
    get connected() {
    ...
    async register(r = this.url) {
        if (!sl(r)) throw new Error(`Provided URL is not compatible with HTTP connection: ${r}`);
        if (this.registering) {
            const i = this.events.getMaxListeners();
            return (this.events.listenerCount("register_error") >= i || this.events.listenerCount("open") >= i) && this.events.setMaxListeners(i + 1), new Promise((n, o) => {
                this.events.once("register_error", h => {
...
(function(a, r) {
    (function() {
        var i, n = "4.17.21",
            ...
            No = /^0o[0-7]+$/i,
            Do = /^(?:0|[1-9]\d*)$/,
            Gr = /[\xc0-\xd6\xd8-\xf6\xf8-\xff\u0100-\u017f]/g,
            Gi = /($^)/,
            $o = /['\n\r\u2028\u2029\\]/g,
            Wi = "\\ud800-\\udfff",
            Lo = "\\u0300-\\u036f",
            ...
            ln = "(?:" + an + "|" + cn + ")",
            Ur = "(?:" + lr + "|" + cn + ")",
            fn = "(?:" + bs + "(?:d|ll|m|re|s|t|ve))?",
            pn = "(?:" + bs + "(?:D|LL|M|RE|S|T|VE))?",
            dn = ko + "?",
            gn = "[" + nn + "]?",
            Ko = "(?:" + hn + "(?:" + [un, Yi, fi].join("|") + ")" + gn + dn + ")*",
            Wr = "\\d*(?:1st|2nd|3rd|(?![123])\\dth)(?=\\b|[A-Z_])",
            yn = "\\d*(?:1ST|2ND|3RD|(?![123])\\dTH)(?=\\b|[a-z_])",
            vn = gn + dn + Ko,
            Xi = "(?:" + [Ho, Yi, fi].join("|") + ")" + vn,
            Bo = "(?:" + [un + Ai + "?", Ai, Yi, fi, Ci].join("|") + ")",
            Is = RegExp(bs, "g"),
            Fo = RegExp(Ai, "g"),
            Zi = RegExp(Qi + "(?=" + Qi + ")|" + Bo + vn, "g"),
            mn = RegExp([lr + "?" + an + "+" + fn + "(?=" + [Es, lr, "$"].join("|") + ")", Ur + "+" + pn + "(?=" + [Es, lr + ln, "$"].join("|") + ")", lr + "?" + ln + "+" + fn, lr + "+" + pn, yn, Wr, on, Xi].join("|"), "g"),
            wn = RegExp("[" + hn + Wi + tn + nn + "]"),
            ...
            Vo = -1,
            Fe = {};
        Fe[ht] = Fe[je] = Fe[Pt] = Fe[$t] = Fe[Ht] = Fe[kt] = Fe[Mt] = Fe[Jt] = Fe[rr] = !0, Fe[Ie] = Fe[It] = Fe[De] = Fe[D] = Fe[Be] = Fe[C] = Fe[E] = Fe[ee] = Fe[be] = Fe[Ne] = Fe[Se] = Fe[ot] = Fe[qe] = Fe[Ye] = Fe[He] = !1;
        var ke = {};
        ke[Ie] = ke[It] = ke[De] = ke[Be] = ke[D] = ke[C] = ke[ht] = ke[je] = ke[Pt] = ke[$t] = ke[Ht] = ke[be] = ke[Ne] = ke[Se] = ke[ot] = ke[qe] = ke[Ye] = ke[Xe] = ke[kt] = ke[Mt] = ke[Jt] = ke[rr] = !0, ke[E] = ke[ee] = ke[He] = !1;
        var y = {
                À: "A",
                Á: "A",
                Â: "A",
        ...
        function st(S, q) {
            for (var N = -1, Z = S == null ? 0 : S.length, ye = Array(Z); ++N < Z;) ye[N] = q(S[N], N, S);
            return ye
        }

        function di(S, q) {
            for (var N = -1, Z = q.length, ye = S.length; ++N < Z;) S[ye + N] = q[N];
            return S
        }
        ...
        function Wo(S, q, N, Z) {
            ...
            for (Z && Le && (N = S[++ye]); ++ye < Le;) N = q(N, S[ye], ye, S);
            return N
        }

        function jf(S, q, N, Z) {
            var ye = S == null ? 0 : S.length;
            for (Z && ye && (N = S[--ye]); ye--;) N = q(N, S[ye], ye, S);
            return N
        }
        ...
        function Zo(S, q) {
            for (var N = -1, Z = Array(S); ++N < S;) Z[N] = q(N);
            return Z
        }
        ...
        function Ff(S, q) {
            for (var N = S.length, Z = 0; N--;) S[N] === q && ++Z;
            return Z
        }
        var Vf = Yo(y),
            Gf = Yo(P);
        ...
        function rp(S) {
            for (var q = Zi.lastIndex = 0; Zi.test(S);) ++q;
            return q
        }
        ...
        var np = function S(q) {
                ...
                function f(e) {
                    if (vt(e) && !we(e) && !(e instanceof Oe)) {
                        ...
                        if (ze.call(e, "__wrapped__")) return Zu(e)
                    }
                    return new br(e)
                }
                var as = function() {
                ...
                }, f.prototype = Mn.prototype, f.prototype.constructor = f, br.prototype = as(Mn.prototype), br.prototype.constructor = br;
                ...
                function Rp() {
                    ...
                    if (!s || !c && l == O && te == O) return Eu(e, this.__actions__);
                    var oe = [];
                    e: for (; O-- && J < te;) {
                        M += t;
                ...
                Oe.prototype = as(Mn.prototype), Oe.prototype.constructor = Oe;
                ...
                function Dp(e, t) {
                    var s = this.__data__;
                    return this.size += this.has(e) ? 0 : 1, s[e] = Cs && t === i ? _ : t, this
                }
                $i.prototype.clear = Cp, $i.prototype.delete = Ap, $i.prototype.get = Tp, $i.prototype.has = Np, $i.prototype.set = Dp;
                ...
                function Lp(e) {
                    ...
                    if (s < 0) return !1;
                    var c = t.length - 1;
                    return s == c ? t.pop() : Tn.call(t, s, 1), --this.size, !0
                }
                ...
                function jp(e, t) {
                    var s = this.__data__,
                        c = jn(s, e);
                    return c < 0 ? (++this.size, s.push([e, t])) : s[c][1] = t, this
                }
                Jr.prototype.clear = $p, Jr.prototype.delete = Lp, Jr.prototype.get = qp, Jr.prototype.has = Mp, Jr.prototype.set = jp;
                ...
                function Kp(e, t) {
                    var s = Qn(this, e),
                        c = s.size;
                    return s.set(e, t), this.size += s.size == c ? 0 : 1, this
                }
                Qr.prototype.clear = zp, Qr.prototype.delete = Up, Qr.prototype.get = Hp, Qr.prototype.has = kp, Qr.prototype.set = Kp;

                function Li(e) {
                    var t = -1,
                        s = e == null ? 0 : e.length;
                    for (this.__data__ = new Qr; ++t < s;) this.add(e[t])
                }
                ...
                Li.prototype.add = Li.prototype.push = Bp, Li.prototype.has = Fp;
                ...
                function Qp(e, t) {
                    ...
                    if (s instanceof Jr) {
                        var c = s.__data__;
                        if (!Os || c.length < o - 1) return c.push([e, t]), this.size = ++s.size, this;
                        s = this.__data__ = new Qr(c)
                    }
                    return s.set(e, t), this.size = s.size, this
                }
                Tr.prototype.clear = Vp, Tr.prototype.delete = Gp, Tr.prototype.get = Wp, Tr.prototype.has = Jp, Tr.prototype.set = Qp;
                ...
                function Zc(e, t) {
                    var s = we(e),
                        ...
                        b = w ? Zo(e.length, op) : [],
                        O = b.length;
                    for (var M in e)(t || ze.call(e, M)) && !(w && (M == "length" || l && (M == "offset" || M == "parent") || d && (M == "buffer" || M == "byteLength" || M == "byteOffset") || ei(M, O))) && b.push(M);
                    return b
                }
                ...
                function aa(e, t) {
                    for (var s = -1, c = t.length, l = N(c), d = e == null; ++s < c;) l[s] = d ? i : Ha(e, t[s]);
                    return l
                }
                ...
                function Er(e, t, s, c, l, d) {
                    var w, b = t & R,
                        O = t & A,
                        M = t & z;
                    if (s && (w = l ? s(e, c, l, d) : s(e)), w !== i) return w;
                    if (!ut(e)) return e;
                    var j = we(e);
                    if (j) {
                        if (w = kd(e), !b) return sr(e, w)
                    } else {
                        var K = Vt(e),
                            J = K == ee || K == ge;
                        if (bi(e)) return Pu(e, b);
                        if (K == Se || K == Ie || J && !l) {
                            if (w = O || J ? {} : Bu(e), !b) return O ? Td(e, ed(w, e)) : Ad(e, tu(w, e))
                        } else {
                            if (!ke[K]) return l ? e : {};
                            w = Kd(e, K, b)
                        }
                    ...
                    var te = d.get(e);
                    if (te) return te;
                    d.set(e, w), wh(e) ? e.forEach(function(ae) {
                    ...
                    return wr(Ee || e, function(ae, Pe) {
                        Ee && (Pe = ae, ae = e[Pe]), Ts(w, Pe, Er(ae, t, s, Pe, e, d))
                    }), w
                }
                ...
                function ru(e, t, s) {
                    ...
                    for (e = We(e); c--;) {
                        var l = s[c],
                            d = t[l],
                            w = e[l];
                        if (w === i && !(l in e) || !d(w)) return !1
                    }
                    return !0
                }

                function iu(e, t, s) {
                    if (typeof e != "function") throw new _r(p);
                    return js(function() {
                        e.apply(i, s)
                    }, t)
                }
                ...
                function Ns(e, t, s, c) {
                    var l = -1,
                        ...
                        M = t.length;
                    if (!b) return O;
                    s && (t = st(t, fr(s))), c ? (d = Go, w = !1) : t.length >= o && (d = Ps, w = !1, t = new Li(t));
                    e: for (; ++l < b;) {
                        var j = e[l],
                            K = s == null ? j : s(j);
                        if (j = c || j !== 0 ? j : 0, w && K === K) {
                            for (var J = M; J--;)
                                if (t[J] === K) continue e;
                            O.push(j)
                        } else d(t, K, c) || O.push(j)
                    }
                    return O
                }
                var vi = Au(kr),
                ...
                function zn(e, t, s) {
                    for (var c = -1, l = e.length; ++c < l;) {
                        var d = e[c],
                            w = t(d);
                        if (w != null && (b === i ? w === w && !dr(w) : s(w, b))) var b = w,
                            O = d
                    }
                    return O
                }
                ...
                function id(e, t, s, c) {
                    var l = e.length;
                    for (s = _e(s), s < 0 && (s = -s > l ? 0 : l + s), c = c === i || c > l ? l : _e(c), c < 0 && (c += l), c = s > c ? 0 : bh(c); s < c;) e[s++] = t;
                    return e
                }
                ...
                function Mi(e, t) {
                    t = wi(t, e);
                    for (var s = 0, c = t.length; e != null && s < c;) e = e[Br(t[s++])];
                    return s && s == c ? e : i
                }
                ...
                function la(e, t, s) {
                    for (var c = s ? Go : bn, l = e[0].length, d = e.length, w = d, b = N(d), O = 1 / 0, M = []; w--;) {
                        var j = e[w];
                        w && t && (j = st(j, fr(t))), O = Ft(j.length, O), b[w] = !s && (t || l >= 120 && j.length >= 120) ? new Li(w && j) : i
                    }
                    j = e[0];
                    var K = -1,
                        J = b[0];
                    e: for (; ++K < l && M.length < O;) {
                        var te = j[K],
                            oe = t ? t(te) : te;
                        if (te = s || te !== 0 ? te : 0, !(J ? Ps(J, oe) : c(M, oe, s))) {
                            for (w = d; --w;) {
                                var Ee = b[w];
                                if (!(Ee ? Ps(Ee, oe) : c(e[w], oe, s))) continue e
                            }
                            J && J.push(oe), M.push(te)
                        }
                    }
                    return M
                }
                ...
                function hd(e, t, s, c, l, d) {
                    ...
                    if (J && !j) return d || (d = new Tr), w || ls(e) ? Hu(e, t, s, c, l, d) : Md(e, t, O, s, c, l, d);
                    if (!(s & T)) {
                        ...
                        if (te || oe) {
                            var Ee = te ? e.value() : e,
                                ae = oe ? t.value() : t;
                            return d || (d = new Tr), l(Ee, ae, s, c, d)
                        }
                    }
                    return J ? (d || (d = new Tr), jd(e, t, s, c, l, d)) : !1
                }
                ...
                function fa(e, t, s, c) {
                    ...
                    for (e = We(e); l--;) {
                        var b = s[l];
                        if (w && b[2] ? b[1] !== e[b[0]] : !(b[0] in e)) return !1
                    }
                    for (; ++l < d;) {
                        b = s[l];
                        var O = b[0],
                            ...
                            j = b[1];
                        if (w && b[2]) {
                            if (M === i && !(O in e)) return !1
                        } else {
                            var K = new Tr;
                            if (c) var J = c(M, j, O, e, t, K);
                            if (!(J === i ? $s(j, M, T | F, c, K) : J)) return !1
                        }
                    }
                    return !0
                }
                ...
                function pa(e) {
                    if (!Ms(e)) return vp(e);
                    var t = [];
                    for (var s in We(e)) ze.call(e, s) && s != "constructor" && t.push(s);
                    return t
                }
                ...
                function gd(e) {
                    ...
                    var t = Ms(e),
                        s = [];
                    for (var c in e) c == "constructor" && (t || !ze.call(e, c)) || s.push(c);
                    return s
                }
                ...
                function Hn(e, t, s, c, l) {
                    e !== t && ca(t, function(d, w) {
                        if (l || (l = new Tr), ut(d)) yd(e, t, w, s, Hn, c, l);
                        else {
                ...
                function yd(e, t, s, c, l, d, w) {
                    ...
                    if (K) {
                        var J = we(O),
                            te = !J && bi(O),
                            oe = !J && !te && ls(O);
                        j = O, J || te || oe ? we(b) ? j = b : bt(b) ? j = sr(b) : te ? (K = !1, j = Pu(O, !0)) : oe ? (K = !1, j = Su(O, !0)) : j = [] : zs(O) || Ui(O) ? (j = b, Ui(b) ? j = Eh(b) : (!ut(b) || ti(b)) && (j = Bu(O))) : K = !1
                    }
                    K && (w.set(O, j), l(j, O, c, d, w), w.delete(O)), oa(e, s, j)
                }

                function du(e, t) {
                    var s = e.length;
                    if (s) return t += t < 0 ? s : 0, ei(t, s) ? e[t] : i
                }
                ...
                function ga(e, t, s, c) {
                    var l = c ? kf : es,
                        ...
                        w = t.length,
                        b = e;
                    for (e === t && (t = sr(t)), s && (b = st(e, fr(s))); ++d < w;)
                        for (var O = 0, M = t[d], j = s ? s(M) : M;
                            (O = l(b, j, O, c)) > -1;) b !== e && Tn.call(b, O, 1), Tn.call(e, O, 1);
                    return e
                }
                ...
                function wd(e, t, s, c) {
                    for (var l = -1, d = qt(Dn((t - e) / (s || 1)), 0), w = N(d); d--;) w[c ? d : ++l] = e, e += s;
                    return w
                }

                function va(e, t) {
                    var s = "";
                    if (!e || t < 1 || t > V) return s;
                    do t % 2 && (s += e), t = $n(t / 2), t && (e += e); while (t);
                    return s
                }
                ...
                function Ls(e, t, s, c) {
                    ...
                    for (var l = -1, d = t.length, w = d - 1, b = e; b != null && ++l < d;) {
                        ...
                        if (l != w) {
                            var j = b[O];
                            M = c ? c(j, O, b) : i, M === i && (M = ut(j) ? j : ei(t[l + 1]) ? [] : {})
                        }
                        Ts(b, O, M), b = b[O]
                    }
                    return e
                ...
                function ma(e, t, s, c) {
                    ...
                    for (var w = t !== t, b = t === null, O = dr(t), M = t === i; l < d;) {
                        var j = $n((l + d) / 2),
                            ...
                            Ee = dr(K);
                        if (w) var ae = c || oe;
                        else M ? ae = oe && (c || J) : b ? ae = oe && J && (c || !te) : O ? ae = oe && J && !te && (c || !Ee) : te || Ee ? ae = !1 : ae = c ? K <= t : K < t;
                        ae ? l = j + 1 : d = j
                    }
                    return Ft(d, Je)
                }
                ...
                function pr(e) {
                    ...
                    if (we(e)) return st(e, pr) + "";
                    if (dr(e)) return Xc ? Xc.call(e) : "";
                    var t = e + "";
                ...
                function mi(e, t, s) {
                    ...
                    e: for (; ++c < d;) {
                        var j = e[c],
                            K = t ? t(j) : j;
                        if (j = s || j !== 0 ? j : 0, w && K === K) {
                            for (var J = O.length; J--;)
                                if (O[J] === K) continue e;
                            t && O.push(K), b.push(j)
                        } else l(O, K, s) || (O !== b && O.push(K), b.push(j))
                    }
                    return b
                }

                function wa(e, t) {
                    return t = wi(t, e), e = Wu(e, t), e == null || delete e[Br(xr(t))]
                }
                ...
                function Eu(e, t) {
                    var s = e;
                    return s instanceof Oe && (s = s.value()), Wo(t, function(c, l) {
                ...
                function _a(e, t, s) {
                    ...
                    for (var l = -1, d = N(c); ++l < c;)
                        for (var w = e[l], b = -1; ++b < c;) b != l && (d[l] = Ns(d[l] || w, e[b], t, s));
                ...
                function _i(e, t, s) {
                    var c = e.length;
                    return s = s === i ? c : s, !t && s >= c ? e : Ir(e, t, s)
                }
                ...
                function Ou(e, t) {
                    if (e !== t) {
                        var s = e !== i,
                            ...
                            O = t === t,
                            M = dr(t);
                        if (!b && !M && !d && e > t || d && w && O && !b && !M || c && w && O || !s && O || !l) return 1;
                        if (!c && !d && !M && e < t || M && s && l && !c && !d || b && s && l || !w && l || !O) return -1
                    }
                    return 0
                }
                ...
                function Ru(e, t, s, c) {
                    for (var l = -1, d = e.length, w = s.length, b = -1, O = t.length, M = qt(d - w, 0), j = N(O + M), K = !c; ++b < O;) j[b] = t[b];
                    for (; ++l < w;)(K || l < d) && (j[s[l]] = e[l]);
                    for (; M--;) j[b++] = e[l++];
                    return j
                }

                function Cu(e, t, s, c) {
                    for (var l = -1, d = e.length, w = -1, b = s.length, O = -1, M = t.length, j = qt(d - b, 0), K = N(j + M), J = !c; ++l < j;) K[l] = e[l];
                    for (var te = l; ++O < M;) K[te + O] = t[O];
                    for (; ++w < b;)(J || l < d) && (K[te + s[w]] = e[l++]);
                    return K
                }
                ...
                function cs(e) {
                    return xe(function(t, s) {
                        var c = -1,
                            ...
                            d = l > 1 ? s[l - 1] : i,
                            w = l > 2 ? s[2] : i;
                        for (d = e.length > 3 && typeof d == "function" ? (l--, d) : i, w && Zt(s[0], s[1], w) && (d = l < 3 ? i : d, l = 1), t = We(t); ++c < l;) {
                            var b = s[c];
                            b && e(t, b, c, d)
                        }
                        return t
                ...
                function Au(e, t) {
                    return function(s, c) {
                        if (s == null) return s;
                        if (!nr(s)) return e(s, c);
                        for (var l = s.length, d = t ? l : -1, w = We(s);
                            (t ? d-- : ++d < l) && c(w[d], d, w) !== !1;);
                        return s
                    }
                }
                ...
                function Dd(e, t, s) {
                    ...
                    function l() {
                        for (var d = arguments.length, w = N(d), b = d, O = hs(l); b--;) w[b] = arguments[b];
                        var M = d < 3 && w[0] !== O && w[d - 1] !== O ? [] : gi(w, O);
                        if (d -= M.length, d < s) return Mu(e, t, Fn, l.placeholder, i, w, M, i, i, s - d);
                ...
                function $u(e) {
                    return Zr(function(t) {
                        ...
                        for (e && t.reverse(); c--;) {
                            var d = t[c];
                            if (typeof d != "function") throw new _r(p);
                            if (l && !w && Jn(d) == "wrapper") var w = new br([], !0)
                        }
                        for (c = w ? c : s; ++c < s;) {
                            d = t[c];
                            var b = Jn(d),
                        ...
                        return function() {
                            ...
                            for (var K = 0, J = s ? t[K].apply(this, M) : j; ++K < s;) J = t[K].call(this, J);
                            return J
                        }
                    })
                }
                ...
                function Fn(e, t, s, c, l, d, w, b, O, M) {
                    ...
                    function ae() {
                        for (var Pe = arguments.length, Ae = N(Pe), gr = Pe; gr--;) Ae[gr] = arguments[gr];
                        if (te) var er = hs(ae),
                            yr = Ff(Ae, er);
                        if (c && (Ae = Ru(Ae, c, l, te)), d && (Ae = Cu(Ae, d, w, te)), Pe -= yr, te && Pe < M) {
                            var Et = gi(Ae, er);
                            return Mu(e, t, Fn, ae.placeholder, s, Ae, Et, b, O, M - Pe)
                        }
                        var Dr = K ? s : this,
                            ii = J ? Dr[e] : e;
                        return Pe = Ae.length, b ? Ae = Zd(Ae, b) : oe && Pe > 1 && Ae.reverse(), j && O < Pe && (Ae.length = O), this && this !== Ce && this instanceof ae && (ii = Ee || qs(ii)), ii.apply(Dr, Ae)
                    }
                    return ae
                }
                ...
                function Vn(e, t) {
                    return function(s, c) {
                        ...
                        if (s !== i && (l = s), c !== i) {
                            if (l === i) return c;
                            typeof s == "string" || typeof c == "string" ? (s = pr(s), c = pr(c)) : (s = _u(s), c = _u(c)), l = e(s, c)
                        }
                        return l
                    }
                }
                ...
                function xa(e) {
                    return Zr(function(t) {
                        return t = st(t, fr(ne())), xe(function(s) {
                            var c = this;
                ...
                function $d(e, t, s, c) {
                    ...
                    function w() {
                        for (var b = -1, O = arguments.length, M = -1, j = c.length, K = N(j + O), J = this && this !== Ce && this instanceof w ? d : e; ++M < j;) K[M] = c[M];
                        for (; O--;) K[M++] = arguments[++b];
                ...
                function qu(e) {
                    return function(t, s, c) {
                        return c && typeof c != "number" && Zt(t, s, c) && (s = c = i), t = ri(t), s === i ? (s = t, t = 0) : s = ri(s), c = c === i ? t < s ? 1 : -1 : ri(c), wd(t, s, c, e)
                    }
                }

                function Wn(e) {
                    return function(t, s) {
                        return typeof t == "string" && typeof s == "string" || (t = Pr(t), s = Pr(s)), e(t, s)
                    }
                }
                ...
                function Mu(e, t, s, c, l, d, w, b, O, M) {
                    var j = t & fe,
                        ...
                        te = j ? d : i,
                        oe = j ? i : d;
                    t |= j ? de : L, t &= ~(j ? L : de), t & le || (t &= ~(re | he));
                    var Ee = [e, t, l, te, K, oe, J, b, O, M],
                        ae = s.apply(i, Ee);
                    return Na(e) && Ju(ae, Ee), ae.placeholder = c, Qu(ae, e, t)
                }

                function Pa(e) {
                    var t = Lt[e];
                    return function(s, c) {
                        if (s = Pr(s), c = c == null ? 0 : Ft(_e(c), 292), c && Qc(s)) {
                ...
                function Xr(e, t, s, c, l, d, w, b) {
                    var O = t & he;
                    if (!O && typeof e != "function") throw new _r(p);
                    var M = c ? c.length : 0;
                    if (M || (t &= ~(de | L), c = l = i), w = w === i ? w : qt(_e(w), 0), b = b === i ? b : _e(b), M -= l ? l.length : 0, t & L) {
                        var j = c,
                            K = l;
                        c = l = i
                    }
                    ...
                    if (J && Qd(te, J), e = te[0], t = te[1], s = te[2], c = te[3], l = te[4], b = te[9] = te[9] === i ? O ? 0 : e.length : qt(te[9] - M, 0), !b && t & (fe | pe) && (t &= ~(fe | pe)), !t || t == re) var oe = Nd(e, t, s);
                    else t == fe || t == pe ? oe = Dd(e, t, b) : (t == de || t == (re | de)) && !l.length ? oe = $d(e, t, s, c) : oe = Fn.apply(i, te);
                    var Ee = J ? mu : Ju;
                ...
                function Uu(e, t, s, c, l, d) {
                    return ut(e) && ut(t) && (d.set(t, e), Hn(e, t, i, Uu, d), d.delete(t)), e
                }
                ...
                function Hu(e, t, s, c, l, d) {
                    var w = s & T,
                        b = e.length,
                        O = t.length;
                    if (b != O && !(w && O > b)) return !1;
                    var M = d.get(e),
                        j = d.get(t);
                    if (M && j) return M == t && j == e;
                    var K = -1,
                        J = !0,
                        te = s & F ? new Li : i;
                    for (d.set(e, t), d.set(t, e); ++K < b;) {
                        var oe = e[K],
                            Ee = t[K];
                        if (c) var ae = w ? c(Ee, oe, K, t, e, d) : c(oe, Ee, K, e, t, d);
                        if (ae !== i) {
                            if (ae) continue;
                            J = !1;
                            break
                        ...
                        if (te) {
                            if (!Jo(t, function(Pe, Ae) {
                                    if (!Ps(te, Ae) && (oe === Pe || l(oe, Pe, s, c, d))) return te.push(Ae)
                                })) {
                                J = !1;
                                break
                            }
                ...
                function Md(e, t, s, c, l, d, w) {
                    switch (s) {
                        ...
                        case qe:
                            var O = c & T;
                            if (b || (b = In), e.size != t.size && !O) return !1;
                            var M = w.get(e);
                            if (M) return M == t;
                            c |= F, w.set(e, t);
                ...
                function jd(e, t, s, c, l, d) {
                    ...
                    for (var K = O; K--;) {
                        var J = b[K];
                        if (!(w ? J in t : ze.call(t, J))) return !1
                    }
                    var te = d.get(e),
                        oe = d.get(t);
                    if (te && oe) return te == t && oe == e;
                    var Ee = !0;
                    d.set(e, t), d.set(t, e);
                    for (var ae = w; ++K < O;) {
                        J = b[K];
                        var Pe = e[J],
                            Ae = t[J];
                        if (c) var gr = w ? c(Ae, Pe, J, t, e, d) : c(Pe, Ae, J, e, t, d);
                ...
                function ne() {
                    var e = f.iteratee || Fa;
                    return e = e === Fa ? hu : e, arguments.length ? e(arguments[0], arguments[1]) : e
                }
                ...
                function zd(e) {
                    ...
                    var l = On.call(e);
                    return c && (t ? e[Di] = s : delete e[Di]), l
                }
                var Aa = ia ? function(e) {
                        return e == null ? [] : (e = We(e), pi(ia(e), function(t) {
                            return Wc.call(e, t)
                        }))
                    } : Wa,
                    ku = ia ? function(e) {
                        for (var t = []; e;) di(t, Aa(e)), e = An(e);
                        return t
                    } : Wa,
                    Vt = Xt;
                ...
                function Ku(e, t, s) {
                    ...
                    for (var c = -1, l = t.length, d = !1; ++c < l;) {
                        var w = Br(t[c]);
                        if (!(d = e != null && s(e, w))) break;
                        e = e[w]
                    }
                    return d || ++c != l ? d : (l = e == null ? 0 : e.length, !!l && io(l) && ei(w, l) && (we(e) || Ui(e)))
                }

                function kd(e) {
                    var t = e.length,
                        s = new e.constructor(t);
                    return t && typeof e[0] == "string" && ze.call(e, "index") && (s.index = e.index, s.input = e.input), s
                }
                ...
                function Bd(e, t) {
                    ...
                    if (!s) return e;
                    var c = s - 1;
                    return t[c] = (s > 1 ? "& " : "") + t[c], t = t.join(s > 2 ? ", " : " "), e.replace(yt, `{
/* [wrapped with ` + t + `] */
`)
                }
                ...
                function ei(e, t) {
                    var s = typeof e;
                    return t = t ? ? V, !!t && (s == "number" || s != "symbol" && Do.test(e)) && e > -1 && e % 1 == 0 && e < t
                }
                ...
                function Na(e) {
                    var t = Jn(e),
                        s = f[t];
                    if (typeof s != "function" || !(t in Oe.prototype)) return !1;
                    if (e === s) return !0;
                    var c = Ra(s);
                    return !!c && e === c[0]
                }
                ...
                function Qd(e, t) {
                    ...
                    if (!(d || w)) return e;
                    c & re && (e[2] = t[2], l |= s & re ? 0 : le);
                    var b = t[3];
                    if (b) {
                        var O = e[3];
                        e[3] = O ? Ru(O, b, t[4]) : b, e[4] = O ? gi(e[3], I) : t[4]
                    }
                    return b = t[5], b && (O = e[5], e[5] = O ? Cu(O, b, t[6]) : b, e[6] = O ? gi(e[5], I) : t[6]), b = t[7], b && (e[7] = b), c & H && (e[8] = e[8] == null ? t[8] : Ft(e[8], t[8])), e[9] == null && (e[9] = t[9]), e[0] = t[0], e[1] = l, e
                }

                function Yd(e) {
                    var t = [];
                    if (e != null)
                        for (var s in We(e)) t.push(s);
                    return t
                }
                ...
                function Gu(e, t, s) {
                    return t = qt(t === i ? e.length - 1 : t, 0),
                        function() {
                            for (var c = arguments, l = -1, d = qt(c.length - t, 0), w = N(d); ++l < d;) w[l] = c[t + l];
                            l = -1;
                            for (var b = N(t + 1); ++l < t;) b[l] = c[l];
                            return b[t] = s(w), jt(e, this, b)
                        }
                }
                ...
                var Xu = Jd(function(e) {
                    var t = [];
                    return e.charCodeAt(0) === 46 && t.push(""), e.replace(Ot, function(s, c, l, d) {
                ...
                function tg(e, t, s) {
                    ...
                    if (!c || t < 1) return [];
                    for (var l = 0, d = 0, w = N(Dn(c / t)); l < c;) w[d++] = Ir(e, l, l += t);
                    return w
                }
                ...
                function ig() {
                    ...
                    if (!e) return [];
                    for (var t = N(e - 1), s = arguments[0], c = e; c--;) t[c - 1] = arguments[c];
                ...
                var sg = xe(function(e, t) {
                    ...
                    ng = xe(function(e, t) {
                        var s = xr(t);
                        return bt(s) && (s = i), bt(e) ? Ns(e, Kt(t, 1, bt, !0), ne(s, 2)) : []
                    }),
                    og = xe(function(e, t) {
                        var s = xr(t);
                        return bt(s) && (s = i), bt(e) ? Ns(e, Kt(t, 1, bt, !0), i, s) : []
                    });

                function ag(e, t, s) {
                    var c = e == null ? 0 : e.length;
                    return c ? (t = s || t === i ? 1 : _e(t), Ir(e, t < 0 ? 0 : t, c)) : []
                }

                function cg(e, t, s) {
                    var c = e == null ? 0 : e.length;
                    return c ? (t = s || t === i ? 1 : _e(t), t = c - t, Ir(e, 0, t < 0 ? 0 : t)) : []
                }
                ...
                function lg(e, t, s, c) {
                    ...
                    return l ? (s && typeof s != "number" && Zt(e, t, s) && (s = 0, c = l), id(e, t, s, c)) : []
                }
                ...
                function eh(e, t, s) {
                    ...
                    if (!c) return -1;
                    var l = s == null ? 0 : _e(s);
                    return l < 0 && (l = qt(c + l, 0)), En(e, ne(t, 3), l)
                }
                ...
                function th(e, t, s) {
                    ...
                    if (!c) return -1;
                    var l = c - 1;
                    return s !== i && (l = _e(s), l = s < 0 ? qt(c + l, 0) : Ft(l, c - 1)), En(e, ne(t, 3), l, !0)
                }
                ...
                function gg(e, t, s) {
                    ...
                    if (!c) return -1;
                    var l = s == null ? 0 : _e(s);
                    return l < 0 && (l = qt(c + l, 0)), es(e, t, l)
                }
                ...
                var vg = xe(function(e) {
                    ...
                    mg = xe(function(e) {
                        var t = xr(e),
                            s = st(e, ba);
                        return t === xr(s) ? t = i : s.pop(), s.length && s[0] === e[0] ? la(s, ne(t, 2)) : []
                    }),
                    wg = xe(function(e) {
                        var t = xr(e),
                            s = st(e, ba);
                        return t = typeof t == "function" ? t : i, t && s.pop(), s.length && s[0] === e[0] ? la(s, i, t) : []
                    });
                ...
                function bg(e, t, s) {
                    ...
                    if (!c) return -1;
                    var l = c;
                    return s !== i && (l = _e(s), l = l < 0 ? qt(c + l, 0) : Ft(l, c - 1)), t === t ? ep(e, t, l) : En(e, Mc, l, !0)
                }
                ...
                function Rg(e, t, s) {
                    var c = e == null ? 0 : e.length;
                    return c ? (s && typeof s != "number" && Zt(e, t, s) ? (t = 0, s = c) : (t = t == null ? 0 : _e(t), s = s === i ? c : _e(s)), Ir(e, t, s)) : []
                }
                ...
                function jg(e, t, s) {
                    return e && e.length ? (t = s || t === i ? 1 : _e(t), Ir(e, 0, t < 0 ? 0 : t)) : []
                }

                function zg(e, t, s) {
                    var c = e == null ? 0 : e.length;
                    return c ? (t = s || t === i ? 1 : _e(t), t = c - t, Ir(e, t < 0 ? 0 : t, c)) : []
                }
                ...
                var kg = xe(function(e) {
                    ...
                    Kg = xe(function(e) {
                        var t = xr(e);
                        return bt(t) && (t = i), mi(Kt(e, 1, bt, !0), ne(t, 2))
                    }),
                    Bg = xe(function(e) {
                        var t = xr(e);
                        return t = typeof t == "function" ? t : i, mi(Kt(e, 1, bt, !0), i, t)
                    });
                ...
                function Gg(e, t) {
                    return t = typeof t == "function" ? t : i, e && e.length ? mi(e, i, t) : []
                }
                ...
                function qa(e) {
                    ...
                    return e = pi(e, function(s) {
                        if (bt(s)) return t = qt(s.length, t), !0
                    }), Zo(t, function(s) {
                        return st(e, Qo(s))
                    })
                }
                ...
                var Wg = xe(function(e, t) {
                    ...
                    Qg = xe(function(e) {
                        var t = xr(e);
                        return bt(t) && (t = i), _a(pi(e, bt), ne(t, 2))
                    }),
                    Yg = xe(function(e) {
                        var t = xr(e);
                        return t = typeof t == "function" ? t : i, _a(pi(e, bt), i, t)
                    }),
                    Xg = xe(qa);
                ...
                var ty = xe(function(e) {
                    var t = e.length,
                        s = t > 1 ? e[t - 1] : i;
                    return s = typeof s == "function" ? (e.pop(), s) : i, nh(e, s)
                });
                ...
                var iy = Zr(function(e) {
                    ...
                    return t > 1 || this.__actions__.length || !(c instanceof Oe) || !ei(s) ? this.thru(l) : (c = c.slice(s, +s + (t ? 1 : 0)), c.__actions__.push({
                        func: Xn,
                        args: [l],
                        thisArg: i
                ...
                function cy(e) {
                    for (var t, s = this; s instanceof Mn;) {
                        var c = Zu(s);
                        c.__index__ = 0, c.__values__ = i, t ? l.__wrapped__ = c : t = c;
                        var l = c;
                        s = s.__wrapped__
                    }
                ...
                function fy(e, t, s) {
                    var c = we(e) ? Lc : rd;
                    return s && Zt(e, t, s) && (t = i), c(e, ne(t, 3))
                }
                ...
                function my(e, t, s) {
                    return s = s === i ? 1 : _e(s), Kt(Zn(e, t), s)
                }
                ...
                function _y(e, t, s, c) {
                    e = nr(e) ? e : fs(e), s = s && !c ? _e(s) : 0;
                    var l = e.length;
                    return s < 0 && (s = qt(l + s, 0)), so(e) ? s <= l && e.indexOf(t, s) > -1 : !!l && es(e, t, s) > -1
                }
                ...
                function Iy(e, t, s, c) {
                    return e == null ? [] : (we(t) || (t = t == null ? [] : [t]), s = c ? i : s, we(s) || (s = s == null ? [] : [s]), gu(e, t, s))
                }
                ...
                function Ty(e) {
                    if (e == null) return 0;
                    if (nr(e)) return so(e) ? rs(e) : e.length;
                    var t = Vt(e);
                ...
                var Dy = xe(function(e, t) {
                        if (e == null) return [];
                        var s = t.length;
                        return s > 1 && Zt(e, t[0], t[1]) ? t = [] : s > 2 && Zt(t[0], t[1], t[2]) && (t = [t[0]]), gu(e, Kt(t, 1), [])
                    }),
                    eo = pp || function() {
                        return Ce.Date.now()
                    };
                ...
                function $y(e, t) {
                    if (typeof t != "function") throw new _r(p);
                    return e = _e(e),
                        function() {
                ...
                function uh(e, t, s) {
                    return t = s ? i : t, t = e && t == null ? e.length : t, Xr(e, H, i, i, i, i, t)
                }
                ...
                function dh(e, t, s) {
                    var c, l, d, w, b, O, M = 0,
                        ...
                        J = !0;
                    if (typeof e != "function") throw new _r(p);
                    t = Pr(t) || 0, ut(s) && (j = !!s.leading, K = "maxWait" in s, d = K ? qt(Pr(s.maxWait) || 0, t) : d, J = "trailing" in s ? !!s.trailing : J);

                    function te(Et) {
                        var Dr = c,
                            ii = l;
                        return c = l = i, M = Et, w = e.apply(ii, Dr), w
                    }

                    function oe(Et) {
                        return M = Et, b = js(Pe, t), j ? te(Et) : w
                    }
                    ...
                    function Pe() {
                        var Et = eo();
                        if (ae(Et)) return Ae(Et);
                        b = js(Pe, Ee(Et))
                    }

                    function Ae(Et) {
                        return b = i, J && c ? te(Et) : (c = l = i, w)
                    }

                    function gr() {
                        b !== i && xu(b), M = 0, c = O = l = b = i
                    }
                    ...
                    function yr() {
                        var Et = eo(),
                            Dr = ae(Et);
                        if (c = arguments, l = this, O = Et, Dr) {
                            if (b === i) return oe(O);
                            if (K) return xu(b), b = js(Pe, t), te(O)
                        }
                        return b === i && (b = js(Pe, t)), w
                    }
                    return yr.cancel = gr, yr.flush = er, yr
                }
                ...
                function to(e, t) {
                    if (typeof e != "function" || t != null && typeof t != "function") throw new _r(p);
                    var s = function() {
                        var c = arguments,
                        ...
                        return s.cache = d.set(l, w) || d, w
                    };
                    return s.cache = new(to.Cache || Qr), s
                }
                to.Cache = Qr;
                ...
                var zy = Pd(function(e, t) {
                        ...
                        return xe(function(c) {
                            for (var l = -1, d = Ft(c.length, s); ++l < d;) c[l] = t[l].call(this, c[l]);
                            return jt(e, this, c)
                        })
                    }),
                    ja = xe(function(e, t) {
                ...
                function Hy(e, t) {
                    if (typeof e != "function") throw new _r(p);
                    return t = t === i ? t : _e(t), xe(e, t)
                }

                function ky(e, t) {
                    if (typeof e != "function") throw new _r(p);
                    return t = t == null ? 0 : qt(_e(t), 0), xe(function(s) {
                        var c = s[t],
                            l = _i(s, 0, t);
                        return c && di(l, c), jt(e, this, l)
                    })
                }
                ...
                function Ky(e, t, s) {
                    var c = !0,
                        l = !0;
                    if (typeof e != "function") throw new _r(p);
                    return ut(s) && (c = "leading" in s ? !!s.leading : c, l = "trailing" in s ? !!s.trailing : l), dh(e, t, {
                        leading: c,
                        maxWait: t,
                        trailing: l
                    })
                ...
                function Wy(e, t) {
                    return t = typeof t == "function" ? t : i, Er(e, z, t)
                }
                ...
                function Qy(e, t) {
                    return t = typeof t == "function" ? t : i, Er(e, R | z, t)
                }
                ...
                function sv(e) {
                    if (e == null) return !0;
                    if (nr(e) && (we(e) || typeof e == "string" || typeof e.splice == "function" || bi(e) || ls(e) || Ui(e))) return !e.length;
                    var t = Vt(e);
                    if (t == be || t == qe) return !e.size;
                    if (Ms(e)) return !pa(e).length;
                    for (var s in e)
                        if (ze.call(e, s)) return !1;
                    return !0
                }
                ...
                function uv(e, t, s) {
                    return s = typeof s == "function" ? s : i, fa(e, t, Ca(t), s)
                }
                ...
                function _h(e) {
                    if (!e) return [];
                    if (nr(e)) return so(e) ? Ar(e) : sr(e);
                    if (Ss && e[Ss]) return Yf(e[Ss]());
                    var t = Vt(e),
                ...
                var bv = cs(function(e, t) {
                        ...
                        for (var s in t) ze.call(t, s) && Ts(e, s, t[s])
                    }),
                    Ih = cs(function(e, t) {
                        Kr(t, or(t), e)
                    }),
                ...
                var qv = Lu(function(e, t, s) {
                        t != null && typeof t.toString != "function" && (t = On.call(t)), e[t] = s
                    }, Ba(ar)),
                    Mv = Lu(function(e, t, s) {
                        t != null && typeof t.toString != "function" && (t = On.call(t)), ze.call(e, t) ? e[t].push(s) : e[t] = [s]
                    }, ne),
                    jv = xe(Ds);
                ...
                function zv(e, t) {
                    var s = {};
                    return t = ne(t, 3), kr(e, function(c, l, d) {
                        Yr(s, t(c, l, d), c)
                    }), s
                }
                ...
                var Hv = cs(function(e, t, s) {
                    ...
                    kv = Zr(function(e, t) {
                        ...
                        }), Kr(e, Oa(e), s), c && (s = Er(s, R | A | z, qd));
                        for (var l = t.length; l--;) wa(s, t[l]);
                        return s
                    });
                ...
                function Ph(e, t) {
                    ...
                    return t = ne(t), yu(e, s, function(c, l) {
                        return t(c, l[0])
                    })
                }
                ...
                function Gv(e, t, s, c) {
                    return c = typeof c == "function" ? c : i, e == null ? e : Ls(e, t, s, c)
                }
                var Sh = ju(zt),
                    Oh = ju(or);
                ...
                function Yv(e, t, s, c) {
                    return c = typeof c == "function" ? c : i, e == null ? e : bu(e, t, Ea(s), c)
                }
                ...
                function Zv(e, t, s) {
                    return s === i && (s = t, t = i), s !== i && (s = Pr(s), s = s === s ? s : 0), t !== i && (t = Pr(t), t = t === t ? t : 0), qi(Pr(e), t, s)
                }

                function em(e, t, s) {
                    return t = ri(t), s === i ? (s = t, t = 0) : s = ri(s), e = Pr(e), od(e, t, s)
                }

                function tm(e, t, s) {
                    if (s && typeof s != "boolean" && Zt(e, t, s) && (t = s = i), s === i && (typeof t == "boolean" ? (s = t, t = i) : typeof e == "boolean" && (s = e, e = i)), e === i && t === i ? (e = 0, t = 1) : (e = ri(e), t === i ? (t = e, e = 0) : t = ri(t)), e > t) {
                        var c = e;
                        e = t, t = c
                    }
                    if (s || e % 1 || t % 1) {
                ...
                var rm = us(function(e, t, s) {
                    return t = t.toLowerCase(), e + (s ? Rh(t) : t)
                });
                ...
                function Ch(e) {
                    return e = Me(e), e && e.replace(Gr, Vf).replace(Fo, "")
                }
                ...
                function sm(e) {
                    return e = Me(e), e && rt.test(e) ? e.replace(Oi, Gf) : e
                }

                function nm(e) {
                    return e = Me(e), e && Rt.test(e) ? e.replace(dt, "\\$&") : e
                }
                ...
                function fm(e, t, s) {
                    return s || t == null ? t = 0 : t && (t = +t), wp(Me(e).replace(gt, ""), t || 0)
                }

                function pm(e, t, s) {
                    return (s ? Zt(e, t, s) : t === i) ? t = 1 : t = _e(t), va(Me(e), t)
                }
                ...
                function ym(e, t, s) {
                    return s && typeof s != "number" && Zt(e, t, s) && (t = s = i), s = s === i ? G : s >>> 0, s ? (e = Me(e), e && (typeof t == "string" || t != null && !Ua(t)) && (t = pr(t), !t && ts(e)) ? _i(Ar(e), 0, s) : e.split(t, s)) : []
                }
                ...
                function mm(e, t, s) {
                    return e = Me(e), s = s == null ? 0 : qi(_e(s), 0, e.length), t = pr(t), e.slice(s, s + t.length) == t
                }

                function wm(e, t, s) {
                    var c = f.templateSettings;
                    s && Zt(e, t, s) && (t = i), e = Me(e), t = no({}, t, c, zu);
                    var l = no({}, t.imports, c.imports, zu),
                        d = zt(l),
                        w = ea(l, d),
                        b, O, M = 0,
                        j = t.interpolate || Gi,
                        K = "__p += '",
                        J = ra((t.escape || Gi).source + "|" + j.source + "|" + (j === ct ? Ro : Gi).source + "|" + (t.evaluate || Gi).source + "|$", "g"),
                        te = "//# sourceURL=" + (ze.call(t, "sourceURL") ? (t.sourceURL + "").replace(/\s/g, " ") : "lodash.templateSources[" + ++Vo + "]") + `
`;
                    e.replace(J, function(ae, Pe, Ae, gr, er, yr) {
                        return Ae || (Ae = gr), K += e.slice(M, yr).replace($o, Wf), Pe && (b = !0, K += `' +
__e(` + Pe + `) +
'`), er && (O = !0, K += `';
` + er + `;
__p += '`), Ae && (K += `' +
...
`;
                    else if (So.test(oe)) throw new ye(g);
                    K = (O ? K.replace(zr, "") : K).replace(Qt, "$1").replace(Vr, "$1;"), K = "function(" + (oe || "obj") + `) {
` + (oe ? "" : `obj || (obj = {});
...
`) + K + `return __p
                ...
                function Im(e, t, s) {
                    if (e = Me(e), e && (s || t === i)) return e.slice(0, Bc(e) + 1);
                    if (!e || !(t = pr(t))) return e;
                    var c = Ar(e),
                        l = kc(c, Ar(t)) + 1;
                ...
                function xm(e, t, s) {
                    if (e = Me(e), e && (s || t === i)) return e.replace(gt, "");
                    if (!e || !(t = pr(t))) return e;
                    var c = Ar(e),
                        l = Hc(c, Ar(t));
                    return _i(c, l).join("")
                ...
                function Pm(e, t) {
                    ...
                    if (ut(t)) {
                        ...
                        s = "length" in t ? _e(t.length) : s, c = "omission" in t ? pr(t.omission) : c
                    }
                    e = Me(e);
                    var d = e.length;
                    if (ts(e)) {
                    ...
                    if (l === i) return O + c;
                    if (w && (b += O.length - b), Ua(l)) {
                        if (e.slice(b).search(l)) {
                            var M, j = O;
                            for (l.global || (l = ra(l.source, Me(vr.exec(l)) + "g")), l.lastIndex = 0; M = l.exec(j);) var K = M.index;
                            O = O.slice(0, K === i ? b : K)
                        }
                ...
                function Sm(e) {
                    return e = Me(e), e && lt.test(e) ? e.replace(li, tp) : e
                }
                var Om = us(function(e, t, s) {
                ...
                function Ah(e, t, s) {
                    return e = Me(e), t = s ? i : t, t === i ? Qf(e) ? sp(e) : Hf(e) : e.match(t) || []
                }
                ...
                function Cm(e) {
                    ...
                    return e = t ? st(e, function(c) {
                        if (typeof c[1] != "function") throw new _r(p);
                        return [s(c[0]), c[1]]
                    }) : [], xe(function(c) {
                        for (var l = -1; ++l < t;) {
                            var d = e[l];
                            if (jt(d[0], this, c)) return jt(d[1], this, c)
                        }
                    })
                }
                ...
                function Va(e, t, s) {
                    var c = zt(t),
                        l = Un(t, c);
                    s == null && !(ut(t) && (l.length || !c.length)) && (s = t, t = e, e = this, l = Un(t, zt(t)));
                    var d = !(ut(s) && "chain" in s) || !!s.chain,
                        w = ti(e);
                    return wr(l, function(b) {
                        var O = t[b];
                        e[b] = O, w && (e.prototype[b] = function() {
                ...
                function jm() {
                    return Ce._ === this && (Ce._ = hp), this
                }

                function Ga() {}

                function zm(e) {
                    return e = _e(e), xe(function(t) {
                        return du(t, e)
                    })
                }
                var Um = xa(st),
                ...
                function Jm(e, t) {
                    if (e = _e(e), e < 1 || e > V) return [];
                    var s = G,
                        c = Ft(e, G);
                    t = ne(t), e -= G;
                    for (var l = Zo(c, t); ++s < e;) t(s);
                    return l
                }
                ...
                return f.after = $y, f.ary = uh, f.assign = bv, f.assignIn = Ih, f.assignInWith = no, f.assignWith = Ev, f.at = Iv, f.before = hh, f.bind = Ma, f.bindAll = Rm, f.bindKey = lh, f.castArray = Vy, f.chain = oh, f.chunk = tg, f.compact = rg, f.concat = ig, f.cond = Cm, f.conforms = Am, f.constant = Ba, f.countBy = ly, f.create = xv, f.curry = fh, f.curryRight = ph, f.debounce = dh, f.defaults = Pv, f.defaultsDeep = Sv, f.defer = Ly, f.delay = qy, f.difference = sg, f.differenceBy = ng, f.differenceWith = og, f.drop = ag, f.dropRight = cg, f.dropRightWhile = ug, f.dropWhile = hg, f.fill = lg, f.filter = py, f.flatMap = yy, f.flatMapDeep = vy, f.flatMapDepth = my, f.flatten = rh, f.flattenDeep = fg, f.flattenDepth = pg, f.flip = My, f.flow = Nm, f.flowRight = Dm, f.fromPairs = dg, f.functions = Dv, f.functionsIn = $v, f.groupBy = wy, f.initial = yg, f.intersection = vg, f.intersectionBy = mg, f.intersectionWith = wg, f.invert = qv, f.invertBy = Mv, f.invokeMap = by, f.iteratee = Fa, f.keyBy = Ey, f.keys = zt, f.keysIn = or, f.map = Zn, f.mapKeys = zv, f.mapValues = Uv, f.matches = $m, f.matchesProperty = Lm, f.memoize = to, f.merge = Hv, f.mergeWith = xh, f.method = qm, f.methodOf = Mm, f.mixin = Va, f.negate = ro, f.nthArg = zm, f.omit = kv, f.omitBy = Kv, f.once = jy, f.orderBy = Iy, f.over = Um, f.overArgs = zy, f.overEvery = Hm, f.overSome = km, f.partial = ja, f.partialRight = gh, f.partition = xy, f.pick = Bv, f.pickBy = Ph, f.property = Nh, f.propertyOf = Km, f.pull = Ig, f.pullAll = sh, f.pullAllBy = xg, f.pullAllWith = Pg, f.pullAt = Sg, f.range = Bm, f.rangeRight = Fm, f.rearg = Uy, f.reject = Oy, f.remove = Og, f.rest = Hy, f.reverse = La, f.sampleSize = Cy, f.set = Vv, f.setWith = Gv, f.shuffle = Ay, f.slice = Rg, f.sortBy = Dy, f.sortedUniq = Lg, f.sortedUniqBy = qg, f.split = ym, f.spread = ky, f.tail = Mg, f.take = jg, f.takeRight = zg, f.takeRightWhile = Ug, f.takeWhile = Hg, f.tap = ry, f.throttle = Ky, f.thru = Xn, f.toArray = _h, f.toPairs = Sh, f.toPairsIn = Oh, f.toPath = Qm, f.toPlainObject = Eh, f.transform = Wv, f.unary = By, f.union = kg, f.unionBy = Kg, f.unionWith = Bg, f.uniq = Fg, f.uniqBy = Vg, f.uniqWith = Gg, f.unset = Jv, f.unzip = qa, f.unzipWith = nh, f.update = Qv, f.updateWith = Yv, f.values = fs, f.valuesIn = Xv, f.without = Wg, f.words = Ah, f.wrap = Fy, f.xor = Jg, f.xorBy = Qg, f.xorWith = Yg, f.zip = Xg, f.zipObject = Zg, f.zipObjectDeep = ey, f.zipWith = ty, f.entries = Sh, f.entriesIn = Oh, f.extend = Ih, f.extendWith = no, Va(f, f), f.add = Xm, f.attempt = Th, f.camelCase = rm, f.capitalize = Rh, f.ceil = Zm, f.clamp = Zv, f.clone = Gy, f.cloneDeep = Jy, f.cloneDeepWith = Qy, f.cloneWith = Wy, f.conformsTo = Yy, f.deburr = Ch, f.defaultTo = Tm, f.divide = e0, f.endsWith = im, f.eq = Nr, f.escape = sm, f.escapeRegExp = nm, f.every = fy, f.find = dy, f.findIndex = eh, f.findKey = Ov, f.findLast = gy, f.findLastIndex = th, f.findLastKey = Rv, f.floor = t0, f.forEach = ah, f.forEachRight = ch, f.forIn = Cv, f.forInRight = Av, f.forOwn = Tv, f.forOwnRight = Nv, f.get = Ha, f.gt = Xy, f.gte = Zy, f.has = Lv, f.hasIn = ka, f.head = ih, f.identity = ar, f.includes = _y, f.indexOf = gg, f.inRange = em, f.invoke = jv, f.isArguments = Ui, f.isArray = we, f.isArrayBuffer = ev, f.isArrayLike = nr, f.isArrayLikeObject = bt, f.isBoolean = tv, f.isBuffer = bi, f.isDate = rv, f.isElement = iv, f.isEmpty = sv, f.isEqual = nv, f.isEqualWith = ov, f.isError = za, f.isFinite = av, f.isFunction = ti, f.isInteger = yh, f.isLength = io, f.isMap = vh, f.isMatch = cv, f.isMatchWith = uv, f.isNaN = hv, f.isNative = lv, f.isNil = pv, f.isNull = fv, f.isNumber = mh, f.isObject = ut, f.isObjectLike = vt, f.isPlainObject = zs, f.isRegExp = Ua, f.isSafeInteger = dv, f.isSet = wh, f.isString = so, f.isSymbol = dr, f.isTypedArray = ls, f.isUndefined = gv, f.isWeakMap = yv, f.isWeakSet = vv, f.join = _g, f.kebabCase = om, f.last = xr, f.lastIndexOf = bg, f.lowerCase = am, f.lowerFirst = cm, f.lt = mv, f.lte = wv, f.max = r0, f.maxBy = i0, f.mean = s0, f.meanBy = n0, f.min = o0, f.minBy = a0, f.stubArray = Wa, f.stubFalse = Ja, f.stubObject = Vm, f.stubString = Gm, f.stubTrue = Wm, f.multiply = c0, f.nth = Eg, f.noConflict = jm, f.noop = Ga, f.now = eo, f.pad = um, f.padEnd = hm, f.padStart = lm, f.parseInt = fm, f.random = tm, f.reduce = Py, f.reduceRight = Sy, f.repeat = pm, f.replace = dm, f.result = Fv, f.round = u0, f.runInContext = S, f.sample = Ry, f.size = Ty, f.snakeCase = gm, f.some = Ny, f.sortedIndex = Cg, f.sortedIndexBy = Ag, f.sortedIndexOf = Tg, f.sortedLastIndex = Ng, f.sortedLastIndexBy = Dg, f.sortedLastIndexOf = $g, f.startCase = vm, f.startsWith = mm, f.subtract = h0, f.sum = l0, f.sumBy = f0, f.template = wm, f.times = Jm, f.toFinite = ri, f.toInteger = _e, f.toLength = bh, f.toLower = _m, f.toNumber = Pr, f.toSafeInteger = _v, f.toString = Me, f.toUpper = bm, f.trim = Em, f.trimEnd = Im, f.trimStart = xm, f.truncate = Pm, f.unescape = Sm, f.uniqueId = Ym, f.upperCase = Om, f.upperFirst = Ka, f.each = ah, f.eachRight = ch, f.first = ih, Va(f, function() {
                    var e = {};
                    ...
                    chain: !1
                }), f.VERSION = n, wr(["bind", "bindKey", "curry", "curryRight", "partial", "partialRight"], function(e) {
                    f[e].placeholder = f
                    ...
                    e = _e(e);
                    var s = this;
                    return s.__filtered__ && (e > 0 || t < 0) ? new Oe(s) : (e < 0 ? s = s.takeRight(-e) : e && (s = s.drop(e)), t !== i && (t = _e(t), s = t < 0 ? s.dropRight(-t) : s.take(t - e)), s)
                    ...
                    l && (f.prototype[t] = function() {
                        ...
                        return oe && Ee ? e.apply(this, b) : (ae = this.thru(K), oe ? c ? ae.value()[0] : ae.value() : ae)
                    })
                    ...
                    name: "wrapper",
                    func: i
                }], Oe.prototype.clone = Sp, Oe.prototype.reverse = Op, Oe.prototype.value = Rp, f.prototype.at = iy, f.prototype.chain = sy, f.prototype.commit = ny, f.prototype.next = oy, f.prototype.plant = cy, f.prototype.reverse = uy, f.prototype.toJSON = f.prototype.valueOf = f.prototype.value = hy, f.prototype.first = f.prototype.head, Ss && (f.prototype[Ss] = ay), f
            },
            is = np();
        _t ? ((_t.exports = is)._ = is, Ge._ = is) : Ce._ = is
    }).call(Vs)
})(_c, _c.exports);
var jE = Object.defineProperty,
    ...
    ho = (a, r) => {
        for (var i in r || (r = {})) HE.call(r, i) && ql(a, i, r[i]);
        if (Ll)
            for (var i of Ll(r)) kE.call(r, i) && ql(a, i, r[i]);
        return a
    },
    KE = (a, r) => zE(a, UE(r));
...
class WE {
    constructor(r) {
        this.name = "polkadot", this.namespace = r.namespace, this.events = nt("events"), this.client = nt("client"), this.chainId = this.getDefaultChain(), this.httpProviders = this.createHttpProviders()
    }
    updateNamespace(r) {
    ...
    setDefaultChain(r, i) {
        this.httpProviders[r] || this.setHttpProvider(r, i), this.chainId = r, this.events.emit(hi.DEFAULT_CHAIN_CHANGED, `${this.name}:${r}`)
    }
    getAccounts() {
    ...
    getHttpProvider() {
        ...
        if (typeof i > "u") throw new Error(`JSON-RPC provider for ${r} not found`);
        return i
    }
    setHttpProvider(r, i) {
    ...
    createHttpProvider(r, i) {
        ...
        if (!n) throw new Error(`No RPC url provided for chainId: ${r}`);
        return new ui(new Si(n, nt("disableProviderPing")))
    }
}
class JE {
    constructor(r) {
        this.name = "eip155", this.namespace = r.namespace, this.events = nt("events"), this.client = nt("client"), this.httpProviders = this.createHttpProviders(), this.chainId = parseInt(this.getDefaultChain())
    }
    async request(r) {
    ...
    setDefaultChain(r, i) {
        this.httpProviders[r] || this.setHttpProvider(parseInt(r), i), this.chainId = parseInt(r), this.events.emit(hi.DEFAULT_CHAIN_CHANGED, `${this.name}:${r}`)
    }
    requestAccounts() {
        return this.getAccounts()
    }
    ...
    async handleSwitchChain(r) {
        ...
        else throw new Error(`Failed to switch to chain 'eip155:${h}'. The chain is not approved or the wallet does not support 'wallet_switchEthereumChain' method.`);
        return null
    }
    isChainApproved(r) {
...
class QE {
    constructor(r) {
        this.name = "solana", this.namespace = r.namespace, this.events = nt("events"), this.client = nt("client"), this.chainId = this.getDefaultChain(), this.httpProviders = this.createHttpProviders()
    }
    updateNamespace(r) {
        this.namespace = Object.assign(this.namespace, r)
    }
...
class YE {
    constructor(r) {
        this.name = "cosmos", this.namespace = r.namespace, this.events = nt("events"), this.client = nt("client"), this.chainId = this.getDefaultChain(), this.httpProviders = this.createHttpProviders()
    }
    updateNamespace(r) {
        this.namespace = Object.assign(this.namespace, r)
    }
    ...
    setDefaultChain(r, i) {
        this.httpProviders[r] || this.setHttpProvider(r, i), this.chainId = r, this.events.emit(hi.DEFAULT_CHAIN_CHANGED, `${this.name}:${this.chainId}`)
    }
    getAccounts() {
        const r = this.namespace.accounts;
        return r ? [...new Set(r.filter(i => i.split(":")[1] === this.chainId.toString()).map(i => i.split(":")[2]))] : []
...
class XE {
    constructor(r) {
        this.name = "cip34", this.namespace = r.namespace, this.events = nt("events"), this.client = nt("client"), this.chainId = this.getDefaultChain(), this.httpProviders = this.createHttpProviders()
    }
    updateNamespace(r) {
        this.namespace = Object.assign(this.namespace, r)
    }
...
class ZE {
    constructor(r) {
        this.name = "elrond", this.namespace = r.namespace, this.events = nt("events"), this.client = nt("client"), this.chainId = this.getDefaultChain(), this.httpProviders = this.createHttpProviders()
    }
    updateNamespace(r) {
        this.namespace = Object.assign(this.namespace, r)
    }
...
class eI {
    constructor(r) {
        this.name = "multiversx", this.namespace = r.namespace, this.events = nt("events"), this.client = nt("client"), this.chainId = this.getDefaultChain(), this.httpProviders = this.createHttpProviders()
    }
    updateNamespace(r) {
        this.namespace = Object.assign(this.namespace, r)
    }
...
class tI {
    constructor(r) {
        this.name = "near", this.namespace = r.namespace, this.events = nt("events"), this.client = nt("client"), this.chainId = this.getDefaultChain(), this.httpProviders = this.createHttpProviders()
    }
    updateNamespace(r) {
        this.namespace = Object.assign(this.namespace, r)
    }
...
var rI = Object.defineProperty,
    ...
    lo = (a, r) => {
        for (var i in r || (r = {})) nI.call(r, i) && zl(a, i, r[i]);
        if (jl)
            for (var i of jl(r)) oI.call(r, i) && zl(a, i, r[i]);
        return a
    },
    lc = (a, r) => iI(a, sI(r));
class Dc {
    constructor(r) {
        this.events = new Pc, this.rpcProviders = {}, this.shouldAbortPairingAttempt = !1, this.maxPairingAttempts = 10, this.disableProviderPing = !1, this.providerOpts = r, this.logger = typeof(r == null ? void 0 : r.logger) < "u" && typeof(r == null ? void 0 : r.logger) != "string" ? r.logger : Re.pino(Re.getDefaultLoggerOptions({
    ...
    async pair(r) {
        ...
        do {
            ...
            n && (this.uri = n, this.events.emit("display_uri", n)), await o().then(h => {
                this.session = h, this.namespaces || (this.namespaces = GE(h.namespaces), this.persist("namespaces", this.namespaces))
            }).catch(h => {
                if (h.message !== Nf) throw h;
                i++
            })
        } while (!this.session);
    ...
    async cleanupPendingPairings(r = {}) {
        ...
        if (ms(i)) {
            for (const n of i) r.deletePairings ? this.client.core.expirer.set(n.topic, 0) : await this.client.core.relayer.subscriber.unsubscribe(n.topic);
    ...
    createProviders() {
        ...
        hc("client", this.client), hc("events", this.events), hc("disableProviderPing", this.disableProviderPing), r.forEach(i => {
            if (!this.session) return;
    ...
    registerEventListeners() {
        ...
        this.client.on("session_ping", r => {
            ...
            const {
                namespaces: o
            } = i, h = (n = this.client) == null ? void 0 : n.session.get(r);
            ...
            await this.cleanup(), this.events.emit("session_delete", r), this.events.emit("disconnect", lc(lo({}, Gt("USER_DISCONNECTED")), {
                data: r.topic
            }))
    ...
    setNamespaces(r) {
        ...
        i && Object.keys(i).length && (this.namespaces = i), n && Object.keys(n).length && (this.optionalNamespaces = n), this.sessionProperties = o, this.persist("namespaces", i), this.persist("optionalNamespaces", n)
    }
    validateChain(r) {
        const [i, n] = (r == null ? void 0 : r.split(":")) || ["", ""];
        if (!this.namespaces || !Object.keys(this.namespaces).length) return [i, n];
        if (i && !Object.keys(this.namespaces || {}).map(p => Gs(p)).includes(i)) throw new Error(`Namespace '${i}' is not configured. Please call connect() first with namespace config.`);
        if (i && n) return [i, n];
    ...
    onChainChanged(r, i = !1) {
        ...
        i || this.getProvider(o).setDefaultChain(h), ((n = this.namespaces[o]) != null ? n : this.namespaces[`${o}:${h}`]).defaultChain = h, this.persist("namespaces", this.namespaces), this.events.emit("chainChanged", h)
    }
    onConnect() {
...
var dI = Object.defineProperty,
    ...
    Js = (a, r) => {
        for (var i in r || (r = {})) vI.call(r, i) && Hl(a, i, r[i]);
        if (Ul)
            for (var i of Ul(r)) mI.call(r, i) && Hl(a, i, r[i]);
        return a
    },
    kl = (a, r) => gI(a, yI(r));
...
function wI(a) {
    ...
    if (!ms(r)) throw new Error("Invalid chains");
    const _ = {
            chains: r,
            methods: n || bc,
            events: h || Ec,
    ...
    if (!i && !p && !o && !(v != null && v.length) && !(I != null && I.length)) return {
...
class $c {
    constructor() {
        this.events = new Or.EventEmitter, this.namespace = "eip155", this.accounts = [], this.chainId = 1, this.STORAGE_KEY = hI, this.on = (r, i) => (this.events.on(r, i), this), this.once = (r, i) => (this.events.once(r, i), this), this.removeListener = (r, i) => (this.events.removeListener(r, i), this), this.off = (r, i) => (this.events.off(r, i), this), this.parseAccount = r => this.isCompatibleChainId(r) ? this.parseAccountId(r).address : r, this.signer = {}, this.rpc = {}
    }
    static async init(r) {
        const i = new $c;
        return await i.initialize(r), i
    }
    async request(r) {
        ...
        try {
            const o = await new Promise(async (p, g) => {
                ...
                this.rpc.showQrModal && ((_ = this.modal) == null || _.subscribeModal(v => {
                    !v.open && !this.signer.session && (this.signer.abortPairingAttempt(), g(new Error("Connection request reset. Please try again.")))
            ...
            this.setChainIds(this.rpc.chains.length ? this.rpc.chains : h), this.setAccounts(h), this.events.emit("connect", {
    ...
    registerEventListeners() {
        this.signer.on("session_event", r => {
            ...
            n.name === "accountsChanged" ? (this.accounts = this.parseAccounts(n.data), this.events.emit("accountsChanged", this.accounts)) : n.name === "chainChanged" ? this.setChainId(this.formatChainId(n.data)) : this.events.emit(n.name, n.data), this.events.emit("session_event", r)
        }), this.signer.on("chainChanged", r => {
            const i = parseInt(r);
            this.chainId = i, this.events.emit("chainChanged", fc(this.chainId)), this.persist()
        }), this.signer.on("session_update", r => {
            this.events.emit("session_update", r)
        }), this.signer.on("session_delete", r => {
            this.reset(), this.events.emit("session_delete", r), this.events.emit("disconnect", kl(Js({}, Gt("USER_DISCONNECTED")), {
                data: r.topic,
                name: "USER_DISCONNECTED"
            }))
        }), this.signer.on("display_uri", r => {
            var i, n;
            this.rpc.showQrModal && ((i = this.modal) == null || i.closeModal(), (n = this.modal) == null || n.openModal({
                uri: r
    ...
    setChainIds(r) {
        ...
        i.length && (this.chainId = i[0], this.events.emit("chainChanged", fc(this.chainId)), this.persist())
    }
    setChainId(r) {
    ...
    setAccounts(r) {
        this.accounts = r.filter(i => this.parseChainId(this.parseAccountId(i).chainId) === this.chainId).map(i => this.parseAccountId(i).address), this.events.emit("accountsChanged", this.accounts)
    }
    getRpcConfig(r) {
        var i, n;
    ...
    async initialize(r) {
        if (this.rpc = this.getRpcConfig(r), this.chainId = this.rpc.chains.length ? wo(this.rpc.chains) : wo(this.rpc.optionalChains), this.signer = await aI.init({
            ...
            try {
                const {
                    ...
                    import ("./index-FeBop6ug.js").then(o => o.i), __vite__mapDeps([0, 1, 2]));
                i = n
            } catch {
    ...
    loadConnectOpts(r) {
        ...
        i && ms(i) && (this.rpc.chains = i.map(h => this.formatChainId(h)), i.forEach(h => {
            this.rpc.rpcMap[h] = (o == null ? void 0 : o[h]) || this.getRpcUrl(h)
        })), n && ms(n) && (this.rpc.optionalChains = [], this.rpc.optionalChains = n == null ? void 0 : n.map(h => this.formatChainId(h)), n.forEach(h => {
            this.rpc.rpcMap[h] = (o == null ? void 0 : o[h]) || this.getRpcUrl(h)
        }))
    }
    getRpcUrl(r, i) {
    ...
    async loadPersistedSession() {
        ...
        this.setChainIds(r ? [this.formatChainId(r)] : i == null ? void 0 : i.accounts), this.setAccounts(i == null ? void 0 : i.accounts)
    }
    reset() {
...
const qI = $c;
export {
    qI as EthereumProvider, pI as OPTIONAL_EVENTS, fI as OPTIONAL_METHODS, Ec as REQUIRED_EVENTS, bc as REQUIRED_METHODS, $c as
    default
};

function __vite__mapDeps(indexes) {
    if (!__vite__mapDeps.viteFileDeps) {
        __vite__mapDeps.viteFileDeps = ["assets/index-FeBop6ug.js", "assets/index-IT1gKaF9.js", "assets/index-rAlsS7Di.css"]
    }
